<script lang="ts">
  ...
  $: canEditThisTrial = originalTrial && (originalTrial.player_id === user_info?.player?.id || hasValidatePermission);
  ...
  <form on:submit|preventDefault={saveTimeTrial}>
    ...
    <Section header="Proofs">
      ...
      {#each proofs as proof (proof.id)}
        {#if proof.deleted}
          <div
            class="proof-item-grouping deleted-proof"
            style="border: 1px solid #7f1d1d; padding: 1rem; margin-top: 1rem; margin-bottom: 1rem; border-radius: 0.25rem; background-color: rgba(127, 29, 29, 0.1);"
          >
            <div class="option">
              <span class="text-red-400">Proof marked for deletion: {proof.url || '(empty)'}</span>
              {#if canEditOrRemoveProof(proof)}
                <Button on:click={() => undoDeleteProof(proof.id)} color="alternative">Undo Delete</Button>
              {/if}
            </div>
          </div>
        {:else}
          <div
            class="proof-item-grouping grid grid-cols-1 lg:grid-cols-2 gap-6"
          ...
          >
            ...
            <div class="space-y-4">
              ...
              {#if proof.url}
                <MediaEmbed url={proof.url} fallbackText="Open Proof Link" embedWidth="100%" embedHeight="300" />
              {:else}
    ...
    <Section header="Save Changes">
      ...
      <Button on:click={() => window.history.back()} color="alternative" extra_classes="ml-2">Cancel</Button>
    </Section>
  </form>
{/if}

    ...
    async function fetchProofsForValidation() {
        ...
        try {
            ...
            if (data && data.proofs) {
                ...
                proofsForValidation = data.proofs.filter((p: ProofWithValidationStatusResponseData) => gameFilter ? p.game === gameFilter : true);
            } else {
                proofsForValidation = [];
            }
        } catch (error: unknown) {
...
<div class="container mx-auto p-4">
    ...
    {:else if proofsForValidation.length === 0}
        <div class="text-center py-8">
            <p class="text-lg text-base-content/60">{$LL.TIME_TRIALS.NO_PROOFS_PENDING_FOR_GAME({ game: gameFilter || $LL.TIME_TRIALS.VALIDATION_GAME_FILTER_ALL() })}</p>
        </div>
    {:else}
        <div class="space-y-6">
                            ...
                            <div class="text-right text-sm">
                                ...
                                {#if proof.time_ms}
                                    <div class="text-base-content/70">
                                        {Math.floor(proof.time_ms / 60000)}:{Math.floor((proof.time_ms % 60000) / 1000).toString().padStart(2, '0')}.{(proof.time_ms % 1000).toString().padStart(3, '0')}
                                    </div>
                                {/if}
                            </div>
                        </div>

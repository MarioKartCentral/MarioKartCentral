  ...
  onMount(async () => {
    ...
    if (tournamentId) {
      await Promise.all([
        ...
        fetch(`/api/tournaments/${tournamentId}/placements`),
      ]).then(async ([tournamentResponse, placementResponse]) => {
        if (tournamentResponse.ok) {
          tournament = await tournamentResponse.json();
        }

        if (placementResponse.ok) {
          placements = await placementResponse.json();
        }
      });
    }
    isLoaded = true;
...
{#key $user}
  {#if isLoaded}
    {#if placements && check_tournament_permission($user, tournament_permissions.manage_placements, placements.tournament_id, tournament.series_id)}

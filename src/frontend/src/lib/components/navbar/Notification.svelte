<script lang="ts">
  ...
  have_unread_notification.subscribe(() => {
    if (fetchCount > 1)
      // ignore initial sub and initial user_info sub
  ...
  async function fetchUnreadNotifications() {
    ...
    if (notifications.length === 0) fetchCount++; // increment since have_unread_notification.subscribe won't trigger if 0 notifs
  }
...
<Dropdown>
  <div class="wrapper">
    {#if notifications.length === 0}
      <DropdownHeader divClass="py-2 px-4 text-white"
    ...
    {#each notifications as { id, type, content_id, content_args, link, created_date, is_read }, idx}
      ...
      {/if}
    {/each}
    <DropdownItem href="/{$page.params.lang}/user/notifications"
      ><div class="text-center">{$LL.NOTIFICATION.SEE_ALL_NOTIFICATIONS()}</div></DropdownItem
    >
    {#if hasUnread}
      <DropdownDivider divClass="my-1 h-px bg-gray-500 dark:bg-gray-600" />
      <DropdownItem on:click={makeAllNotificationsAsRead}
        ><div class="text-center">{$LL.NOTIFICATION.MARK_ALL_READ()}</div></DropdownItem
      >
    {/if}
  </div>
</Dropdown>

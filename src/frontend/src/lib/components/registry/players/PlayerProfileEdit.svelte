...
{#if player.id === user_info.player?.id}
  ...
  <Section header={$LL.LOGIN.CHANGE_PASSWORD()}>
    <RegisterForm is_reset is_change bind:old_password bind:password={new_password} on:submit={changePassword} />
  </Section>
{/if}
  ...
  {#if player.user_settings}
    <form method="post" on:submit|preventDefault={user_info.player?.id === player.id ? editProfile : forceEditProfile}>
      <div class="option">
        ...
        <br />
        <textarea name="about_me" maxlength="500"
          >{player.user_settings?.about_me ? player.user_settings.about_me : ''}</textarea
        >
      </div>
      <div class="option">
      ...
      <div class="button">
        <Button type="submit" disabled={!check_permission(user_info, permissions.edit_profile, true)}

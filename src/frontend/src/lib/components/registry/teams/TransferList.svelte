<script lang="ts">
  ...
  async function approveTransfer(transfer: TeamTransfer) {
    ...
    if (res.status < 300) {
      ...
      alert(`${$LL.MODERATOR.APPROVE_TRANSFER_FAILED()}: ${title}`);
    }
  }
  ...
  async function denyTransfer(transfer: TeamTransfer) {
    ...
    if (res.status < 300) {
      ...
      const { title } = await res.json();
      alert(`${$LL.MODERATOR.DENY_TRANSFER_FAILED()}: ${title}`);
    }
  }
      ...
      <div class="row">
        ...
        <div class="right field">
          {#if transfer.roster_leave}
            <a
              href="/{$page.params.lang}/registry/teams/profile?id={transfer.roster_leave.team_id}"
              title={transfer.roster_leave.roster_name}
            >
          ...
          {#if transfer.roster_join}
            <a
              href="/{$page.params.lang}/registry/teams/profile?id={transfer.roster_join.team_id}"
              title={transfer.roster_join.roster_name}
            >

<script lang="ts">
    ...
    export let logo_url: string | null = null;
    export let remove_logo = false;
</script>

{#if logo_url && !remove_logo}
    <div>
        <CancelButton on:click={() => remove_logo = true}/>
    </div>
    <div>
...
{#if !logo_url || remove_logo}
    <div>
        <ImageInput bind:file={file} name="logo"/>
    </div>
{/if}

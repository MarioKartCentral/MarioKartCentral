<script lang="ts">
    ...
    onMount(async() => {
        const res = await fetch(`/api/registry/teams/getRegisterable?tournament_id=${tournament.id}&game=${tournament.game}&mode=${tournament.mode}`);
        if(res.status < 300) {
        ...
        {#if captain_player}
            ...
            <div>
                ...
                {#if captain_player.player_id !== player.id}
                    <button on:click={() => captain_player = null}>
                        X
                    </button>
                {/if}
            </div>
            {#if representatives.length}
                ...
                {#each representatives as player}
                    <div>
                        {player.name}
                        <button on:click={() => removeRep(player)}>
                            X
                        </button>
                    </div>
                {/each}

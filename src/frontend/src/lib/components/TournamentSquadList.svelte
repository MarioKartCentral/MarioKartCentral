<script lang="ts">
    ...
    let all_toggle_on: boolean = false;
    ...
    <thead>
        <tr>
            ...
            {#if tournament.squad_name_required}
                <th>Name</th>
            {/if}
            <th>Players <button class="show-players" on:click={toggle_all_players}>({all_toggle_on ? "hide all" : "show all"})</button></th>
            <th>Eligible?</th>
            ...
            {#if squad_data[squad.id].display_players}
                <tr class="squad-row-{i % 2}">
                    <td colspan="10">
                        ...
                        <div transition:slide={{ duration: 400 }}><TournamentPlayerList tournament={tournament} players={squad.players}/></div>
                    </td>
                </tr>
            {/if}
        {/each}

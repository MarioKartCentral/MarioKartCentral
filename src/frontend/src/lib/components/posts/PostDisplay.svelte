<script lang="ts">
  ...
  import {
    check_permission,
    check_series_permission,
    check_tournament_permission,
    permissions,
    series_permissions,
  ...
  <Section header={post.title}>
    <div slot="header_content">
      {#if tournament_id && check_tournament_permission(user_info, tournament_permissions.manage_tournament_posts, tournament_id, series_id)}
        <Button href="/{$page.params.lang}/tournaments/posts/edit?tournament_id={tournament_id}&id={id}"
      ...
      {:else if series_id && check_series_permission(user_info, series_permissions.manage_series_posts, series_id)}
        <Button href="/{$page.params.lang}/tournaments/series/posts/edit?series_id={series_id}&id={id}"
          >{$LL.COMMON.EDIT()}</Button
        >

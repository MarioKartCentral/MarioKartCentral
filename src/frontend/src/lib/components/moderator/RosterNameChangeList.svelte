<script lang="ts">
  ...
  async function fetchData() {
    const roster_res = await fetch(
      `/api/registry/teams/rosterChangeRequests?approval_status=${approval_status}&page=${currentPage}`,
    );
...
{#if roster_requests.length}
  ...
  <Table data={roster_requests} let:item={request}>
    ...
    <tr class="row">
      ...
      {#if approval_status === 'pending'}
        <td>
          <ConfirmButton on:click={() => approveRosterRequest(request)} />
          <CancelButton on:click={() => denyRosterRequest(request)} />
        </td>
      {/if}
      {#if approval_status !== 'pending'}
      ...
      {#if approval_status === 'approved'}
        <td>
          <Button on:click={() => denyRosterRequest(request)}>{$LL.COMMON.DELETE()}</Button>
        </td>
      {/if}
    </tr>
  </Table>

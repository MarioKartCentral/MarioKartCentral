...
<Table data={banInfoDetailedArray} let:item={bid}>
  ...
  <tr slot="header">
    ...
    <th class="mobile-hide"
      >{#if isHistorical}
        {$LL.PLAYER_BAN.UNBANNED()}
      {:else}
        {$LL.PLAYER_BAN.EXPIRES()}
      {/if}</th
    >
  ...
  <tr class="row">
    ...
    {:else}
      <td class="mobile-hide"
        >{bid.is_indefinite ? $LL.PLAYER_BAN.INDEFINITE() : datetimeToString(bid.expiration_date)}</td
      >
    {/if}
    <td>{bid.reason}</td>
    {#if bid.banned_by_pid}
      <td class="mobile-hide"
        ><a href="/{$page.params.lang}/registry/players/profile?id={bid.banned_by_pid}">{bid.banned_by_name}</a></td
      >
    {:else}
    ...
    {#if isHistorical}
      {#if bid.unbanned_by_pid !== null}
        <td class="mobile-hide"
          ><a href="/{$page.params.lang}/registry/players/profile?id={bid.unbanned_by_pid}">{bid.unbanned_by_name}</a
          ></td
        >

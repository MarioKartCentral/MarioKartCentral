<script lang="ts">
  ...
  async function banPlayer(event: SubmitEvent & { currentTarget: EventTarget & HTMLFormElement }) {
    ...
    if (isEditBan) confirmText = `Are you sure you want to edit the ban for ${playerName}?`;
    ...
    if (response.status < 300) {
      if (isEditBan) alert(`Successfully edited the ban for ${playerName} (Player ID: ${playerId})`);
      else alert(`Successfully banned ${playerName} (Player ID: ${playerId})`);
...
<div>
  <h2>
    {#if isEditBan}
      {$LL.PLAYER_BAN.EDIT_BAN_DETAILS()}
    {:else}
      {$LL.PLAYER_BAN.BAN_DETAILS()}
    {/if}
  </h2>
  <form on:submit|preventDefault={banPlayer}>
    <div>
      <label for="duration"
        >{$LL.PLAYER_BAN.DURATION()}
        {#if isEditBan}({$LL.PLAYER_BAN.FROM_INITIAL_BAN_DATE()}){/if}</label
      > <br />
      <select
        name="duration"
        value={isIndefinite ? 'indefinite' : 'number of days'}
    ...
    <div>
      ...
      {#if reason === 'Other'}
        <input
          name="custom_reason"
          type="text"
          placeholder={$LL.PLAYER_BAN.ENTER_REASON()}
          value={customReason}
          required
        />
      {/if}

...
class DatabaseBackupJob(Job):
    ...
    async def run(self):
        ...
        logging.info(
            f"Database backup completed for set: {backup_set_prefix}. {len(backup_results)} database(s) backed up."
        )
...
class DatabaseBackupCleanupJob(Job):
    ...
    async def run(self):
        ...
        if deleted_backup_set_prefixes:
            logging.info(
                f"""Removed {len(deleted_backup_set_prefixes)} old backup sets (prefixes):
                {", ".join(deleted_backup_set_prefixes)}"""
            )
        else:

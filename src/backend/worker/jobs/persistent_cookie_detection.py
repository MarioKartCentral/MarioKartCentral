...
@dataclass
class PersistentCookieDetectionState:
    last_login_id: int = -1
...
class DetectPersistentCookieMatchesCommand(Command[PersistentCookieDetectionState]):
    ...
    async def handle(self, db_wrapper: DBWrapper) -> PersistentCookieDetectionState:
        ...
        new_state = PersistentCookieDetectionState(last_login_id=max_login_id)

        async with db_wrapper.connect(db_name="alt_flags", attach=["user_activity"]) as db:
            potential_matches_query = """
                SELECT l1.user_id AS user_id_1, l2.user_id AS user_id_2, MIN(l1.date) AS date_1,
                    MIN(l2.date) AS date_2, MIN(l1.id) AS login_id_1, MIN(l2.id) AS login_id_2
                ...
                WHERE
                    ...
                    NOT EXISTS (
                        SELECT 1
                        FROM alt_flags
                        WHERE type = 'persistent_cookie_match' AND
                            flag_key = 'user_id_1=' || l1.user_id || ',user_id_2=' || l2.user_id
                    )
                GROUP BY l1.user_id, l2.user_id
            """

            potential_matches = await db.execute_fetchall(
                potential_matches_query,
            ...
            insert_user_alt_flags_query = """
                WITH new_flags AS (
                    SELECT id, json_extract(data, '$.user_id_1') AS user_id_1,
                        json_extract(data, '$.user_id_2') AS user_id_2
                    FROM alt_flags
                    WHERE type = 'persistent_cookie_match' AND id > :prev_max_flag_id
                ),
                new_user_flags AS (
            ...
            await db.execute(
                insert_user_alt_flags_query,
                {"prev_max_flag_id": prev_max_flag_id},
            )
            await db.commit()
...
class PersistentCookieDetectionJob(Job):
    @property
    def name(self):
        return "Detect Users with matching persistent cookies"
    ...
    async def run(self):
        ...
        state = await self.get_state(PersistentCookieDetectionState)
        if not state:
            state = PersistentCookieDetectionState()

        # Run the command with the state object
        new_state = await handle(DetectPersistentCookieMatchesCommand(state=state))
...
def get_jobs():
    if not _jobs:
        _jobs.append(PersistentCookieDetectionJob())
    return _jobs

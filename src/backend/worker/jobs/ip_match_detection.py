        ...
        async with db_wrapper.connect(db_name="alt_flags", attach=["user_activity"]) as db:
            ...
            matches_query = """
                WITH ip_matches AS (
                    ...
                    WHERE
                        (
                            ip.checked_at > :last_checked_timestamp OR ui1.id > :last_user_ip_id OR
                            ui2.id > :last_user_ip_id
                        ) AND
                ...
                SELECT im.user_id_1, im.user_id_2, im.ip_address_id, im.score, im.flag_key,
                    im.match_date, im.date_1, im.date_2, im.is_mobile, im.is_vpn, im.country,
                    im.region, im.asn
                FROM ip_matches im

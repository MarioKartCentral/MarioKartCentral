...
async def create_my_squad(request: Request, body: CreateSquadRequestData) -> JSONResponse:
    ...
    player_host_permission = await handle(
        CheckUserHasPermissionCommand(
    ...
    return JSONResponse(
        ...
        status_code=201,
        headers={"Location": f"/api/tournaments/{tournament_id}/squads/{registration_id}"},
    )
...
async def register_my_team(request: Request, body: RegisterTeamRequestData) -> JSONResponse:
    tournament_id = request.path_params["tournament_id"]
    player_id = request.state.user.player_id
    command = RegisterTeamTournamentCommand(
        tournament_id,
        body.squad_name,
        body.squad_tag,
        body.squad_color,
        ...
        if registration_id:
            ...
            await handle(
                DispatchNotificationCommand(
                    ...
                    notifications.STAFF_REGISTER_TEAM,
                    {"tournament_name": data.tournament_name},
                    f"/tournaments/details?id={tournament_id}",
...
async def force_create_squad(request: Request, body: ForceCreateSquadRequestData) -> JSONResponse:
    ...
    async def notify(tournament_id: int):
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.STAFF_CREATE_SQUAD,
...
async def invite_player(request: Request, body: InvitePlayerRequestData) -> JSONResponse:
    async def notify():
        ...
        content_args = {
            "squad_name": data.squad_name or "",
            "tournament_name": data.tournament_name,
        }
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.TOURNAMENT_INVITE,
                content_args,
                "/registry/invites",
                notifications.SUCCESS,
            )
    ...
    async def notify(tournament_id: int):
        ...
        if body.registration_id:
            ...
            await handle(
                DispatchNotificationCommand(
                    [data.captain_user_id],
                    notifications.TOURNAMENT_STAFF_REGISTERED_CAPTAIN_NOTIF,
                    content_args,
                    f"/tournaments/details?id={tournament_id}",
                    notifications.INFO,
                )
            ...
            await handle(
                DispatchNotificationCommand(
                    [user_id],
                    notifications.TOURNAMENT_STAFF_REGISTERED,
                    {"tournament_name": data.tournament_name},
                    f"/tournaments/details?id={tournament_id}",
                    notifications.SUCCESS,
                )
...
async def edit_registration(request: Request, body: EditPlayerRegistrationRequestData) -> JSONResponse:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.STAFF_EDIT_PLAYER_REGISTRATION,
                {"tournament_name": tournament_name},
                f"/tournaments/details?id={tournament_id}",
                notifications.INFO,
            )
    ...
    tournament_id = request.path_params["tournament_id"]
    command = EditPlayerRegistrationCommand(
        tournament_id,
        body.registration_id,
        body.player_id,
        body.mii_name,
...
async def accept_invite(request: Request, body: AcceptInviteRequestData) -> JSONResponse:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [data.captain_user_id],
                notifications.TOURNAMENT_INVITE_ACCEPTED,
                content_args,
                f"/tournaments/details?id={tournament_id}",
                notifications.SUCCESS,
            )
...
async def decline_invite(request: Request, body: DeclineInviteRequestData) -> Response:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [data.captain_user_id],
                notifications.DECLINE_SQUAD_INVITE,
                content_args,
                f"/tournaments/details?id={tournament_id}",
...
async def remove_player_from_squad(request: Request, body: KickSquadPlayerRequestData) -> Response:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.TOURNAMENT_KICKED,
                content_args,
                f"/tournaments/details?id={tournament_id}",
                notifications.WARNING,
            )
    ...
    async def notify(tournament_id: int):
        ...
        if body.registration_id:
            ...
            await handle(
                DispatchNotificationCommand(
                    [data.captain_user_id],
                    notifications.TOURNAMENT_STAFF_UNREGISTERED_CAPTAIN_NOTIF,
                    content_args,
                    f"/tournaments/details?id={tournament_id}",
                    notifications.WARNING,
                )
            ...
            await handle(
                DispatchNotificationCommand(
                    [user_id],
                    notifications.TOURNAMENT_STAFF_UNREGISTERED,
                    {"tournament_name": data.tournament_name},
                    f"/tournaments/details?id={tournament_id}",
                    notifications.WARNING,
                )
...
async def change_squad_captain(request: Request, body: MakeCaptainRequestData) -> Response:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.CHANGE_SQUAD_CAPTAIN,
                content_args,
                f"/tournaments/details?id={tournament_id}",
                notifications.SUCCESS,
            )
...
async def add_team_representative(request: Request, body: MakeCaptainRequestData) -> Response:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.ADD_REPRESENTATIVE,
                content_args,
                f"/tournaments/details?id={tournament_id}",
                notifications.SUCCESS,
            )
...
async def remove_team_representative(request: Request, body: MakeCaptainRequestData) -> Response:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.REMOVE_REPRESENTATIVE,
                content_args,
                f"/tournaments/details?id={tournament_id}",
                notifications.WARNING,
            )
...
async def force_unregister_squad(request: Request, body: UnregisterSquadRequestData) -> Response:
    ...
    async def notify(tournament_id: int):
        await handle(
            DispatchNotificationCommand(
                [data.captain_user_id],
                notifications.STAFF_UNREGISTER_TEAM,
                {"tournament_name": data.tournament_name},
                f"/tournaments/details?id={tournament_id}",
                notifications.WARNING,
            )
...
routes = [
    Route(
        "/api/tournaments/{tournament_id:int}/register",
        register_me,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/forceRegister",
        force_register_player,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/editRegistration",
        edit_registration,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/editMyRegistration",
        edit_my_registration,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/createSquad",
        create_my_squad,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/registerTeam",
        register_my_team,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/forceRegisterTeam",
        force_register_team,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/forceCreateSquad",
        force_create_squad,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/editSquad",
        edit_squad,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/editMySquad",
        edit_my_squad,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/invitePlayer",
        invite_player,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/acceptInvite",
        accept_invite,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/declineInvite",
        decline_invite,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/kickPlayer",
        remove_player_from_squad,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/unregister",
        unregister_me,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/forceUnregister",
        staff_unregister,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/makeCaptain",
        change_squad_captain,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/addRepresentative",
        add_team_representative,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/removeRepresentative",
        remove_team_representative,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/unregisterSquad",
        unregister_squad,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/forceUnregisterSquad",
        force_unregister_squad,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/squads/{registration_id:int}",
        view_squad,
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/registrations",
        list_registrations,
    ),
    Route("/api/tournaments/{tournament_id:int}/myRegistration", my_registration),
    Route(
        "/api/tournaments/{tournament_id:int}/toggleCheckin",
        toggle_checkin,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/addRosterToSquad",
        add_roster_to_squad,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/removeRosterFromSquad",
        remove_roster_from_squad,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/forceAddRosterToSquad",
        force_add_roster_to_squad,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/forceRemoveRosterFromSquad",
        force_remove_roster_from_squad,
        methods=["POST"],
    ),
]

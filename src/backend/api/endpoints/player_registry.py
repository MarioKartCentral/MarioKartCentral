    ...
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.FORCE_ADD_FRIEND_CODE,
                {"type": body.type},
...
async def force_edit_fc(request: Request, body: ForceEditFriendCodeRequestData) -> JSONResponse:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.FORCE_EDIT_FRIEND_CODE,
                {"type": type},
                f"/registry/players/profile?id={body.player_id}",
                notifications.INFO,
            )
...
async def force_primary_fc(request: Request, body: ModEditPrimaryFriendCodeRequestData) -> Response:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.FORCE_PRIMARY_FRIEND_CODE,
                {},
                f"/registry/players/profile?id={body.player_id}",
                notifications.INFO,
            )
...
async def approve_player_name_request(request: Request, body: ApprovePlayerNameRequestData) -> JSONResponse:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [data.user_id],
                notifications.NAME_CHANGE_APPROVED,
                {},
...
async def deny_player_name_request(request: Request, body: ApprovePlayerNameRequestData) -> JSONResponse:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [data.user_id],
                notifications.NAME_CHANGE_DENIED,
                {},
                f"/registry/players/profile?id={data.player_id}",
...
async def approve_player_claim(request: Request, body: ApproveDenyPlayerClaimRequestData) -> Response:
    ...
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.PLAYER_CLAIM_APPROVED,
                content_args,
...
async def deny_player_claim(request: Request, body: ApproveDenyPlayerClaimRequestData) -> Response:
    ...
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [user_id],
                notifications.PLAYER_CLAIM_DENIED,
                content_args,
                f"/registry/players/profile?id={player_id}",
                notifications.WARNING,
            )
...
routes = [
    ...
    Route(
        "/api/registry/players/createShadowPlayer",
    ...
    Route(
        "/api/registry/players/{id:int}/notes",
        update_player_notes,
        methods=["POST"],
    ),
    Route("/api/registry/players/{id:int}/lounge", player_lounge),
    Route(
        "/api/registry/players/{player_id:int}/getPlayerTransferHistory",
    ...
    Route(
        "/api/registry/forcePrimaryFriendCode",
        force_primary_fc,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/registry/players/requestName",
        request_edit_player_name,
        methods=["POST"],
    ),
    Route("/api/registry/players/nameChanges", get_player_name_requests),
    Route(
        "/api/registry/players/approveNameChange",
        approve_player_name_request,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/registry/players/denyNameChange",
        deny_player_name_request,
        methods=["POST"],
    ),  # dispatches notification
    Route("/api/registry/players/claim", claim_player, methods=["POST"]),
    Route(
        "/api/registry/players/approveClaim",
    ...
    Route(
        "/api/registry/players/{player_id:int}/toggleTransferItemVisibility/{item_id:int}",
        toggle_transfer_item_visibility,
        methods=["POST"],
    ),
]

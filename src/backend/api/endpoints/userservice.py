...
from common.data.commands import GetUserDataFromIdCommand, GetUserPlayerDataFromIdCommand
...
async def current_user_and_player(request: Request) -> JSONResponse:
    user = await handle(GetUserPlayerDataFromIdCommand(request.state.user.id))
    return JSONResponse(user)

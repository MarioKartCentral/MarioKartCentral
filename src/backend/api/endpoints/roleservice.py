...
async def remove_team_role_from_player(request: Request, body: RemoveRoleRequestData) -> JSONResponse:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [uid],
                notifications.TEAM_ROLE_REMOVE,
                content_args,
                f"/registry/teams/profile?id={team_id}",
                notifications.WARNING,
            )
...
async def grant_series_role_to_player(request: Request, body: GrantRoleRequestData) -> JSONResponse:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [uid],
                notifications.SERIES_ROLE_ADD,
                content_args,
                f"/tournaments/series/details?id={series_id}",
                notif_type,
            )
        )
...
async def remove_series_role_from_player(request: Request, body: RemoveRoleRequestData) -> JSONResponse:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [uid],
                notifications.SERIES_ROLE_REMOVE,
                content_args,
                f"/tournaments/series/details?id={series_id}",
                notifications.WARNING,
            )
...
async def grant_tournament_role_to_player(request: Request, body: GrantRoleRequestData) -> JSONResponse:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [uid],
                notifications.TOURNAMENT_ROLE_ADD,
                content_args,
...
async def remove_tournament_role_from_player(request: Request, body: RemoveRoleRequestData) -> JSONResponse:
    async def notify():
        ...
        await handle(
            DispatchNotificationCommand(
                [uid],
                notifications.TOURNAMENT_ROLE_REMOVE,
                content_args,
                f"/tournaments/details?id={tournament_id}",
                notifications.WARNING,
            )
...
routes = [
    ...
    Route(
        "/api/registry/teams/{team_id:int}/roles/grant",
        grant_team_role_to_player,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/registry/teams/{team_id:int}/roles/remove",
        remove_team_role_from_player,
        methods=["POST"],
    ),  # dispatches notification
    Route("/api/tournaments/series/roles", list_series_roles),
    Route(
        "/api/tournaments/series/{series_id:int}/roles/{role_id:int}",
        series_role_info,
    ),
    Route(
        "/api/tournaments/series/{series_id:int}/roles/grant",
        grant_series_role_to_player,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/series/{series_id:int}/roles/remove",
        remove_series_role_from_player,
        methods=["POST"],
    ),  # dispatches notification
    Route("/api/tournaments/roles", list_tournament_roles),
    Route(
        "/api/tournaments/{tournament_id:int}/roles/{role_id:int}",
        tournament_role_info,
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/roles/grant",
        grant_tournament_role_to_player,
        methods=["POST"],
    ),  # dispatches notification
    Route(
        "/api/tournaments/{tournament_id:int}/roles/remove",
        remove_tournament_role_from_player,
        methods=["POST"],
    ),  # dispatches notification
]

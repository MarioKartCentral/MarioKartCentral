...
from common.data.models import (
    PlayerBanRequestData,
    PlayerBanFilter,
    PlayerBanHistoricalFilter,
)
...
async def edit_player_ban(request: Request, body: PlayerBanRequestData) -> Response:
    async def notify():
        ...
        if ban_list.ban_list:
            ...
            if (
                cur_ban.reason != body.reason
                or cur_ban.is_indefinite != body.is_indefinite
                or cur_ban.expiration_date != body.expiration_date
            ):
                unban_date_text = "Indefinite" if body.is_indefinite else f"DATE-{body.expiration_date}"
...
routes = [
    ...
    Route(
        "/api/registry/players/{id:int}/ban",
        edit_player_ban,
        methods=["PATCH"],

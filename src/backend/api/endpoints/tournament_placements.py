...
@require_tournament_permission(tournament_permissions.MANAGE_PLACEMENTS)
async def set_placements_from_player_ids(
    request: Request, body: list[TournamentPlacementFromPlayerIDs]
) -> JSONResponse:
    tournament_id = int(request.path_params["tournament_id"])
...
async def get_latest_tournament_with_placements(
    request: Request,
) -> JSONResponse:
...
routes = [
    Route(
        "/api/tournaments/{tournament_id:int}/placements/set",
        set_placements,
        methods=["POST"],
    ),
    Route(
        "/api/tournaments/{tournament_id:int}/placements/setFromPlayerIDs",
        set_placements_from_player_ids,
        methods=["POST"],
    ),
    Route("/api/tournaments/{tournament_id:int}/placements", get_placements),
    Route(
        "/api/tournaments/latestWithPlacements",
        get_latest_tournament_with_placements,
    ),
    Route(
        "/api/tournaments/players/placements/{player_id:int}",
        get_player_placements,
    ),

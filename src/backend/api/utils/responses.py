...
def bind_request_query[T](type: type[T]):
    def decorator[**P](handle_request: Callable[Concatenate[Request[State], T, P], Awaitable[Response]]):
        async def wrapper(request: Request[State], *args: P.args, **kwargs: P.kwargs):
            ...
            try:
                body = msgspec.json.decode(
                    query_as_json, type=type, strict=False)
            except msgspec.ValidationError as e:
                raise Problem("Invalid query string parameter",
        ...
        if hasattr(handle_request, 'spec_types'):
            existing_spec_types: RouteSpecTypes = getattr(
                handle_request, 'spec_types')
...
def bind_request_body[T](type: type[T]):
    def decorator[**P](handle_request: Callable[Concatenate[Request[State], T, P], Awaitable[Response]]):
        async def wrapper(request: Request[State], *args: P.args, **kwargs: P.kwargs) -> Response:

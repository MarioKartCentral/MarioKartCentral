...
def get_user_info[**P](
    handle_request: Callable[Concatenate[Request, P], Awaitable[Response]],
):
...
def require_logged_in(session_only: bool = False):
    def require_logged_in_decorator[**P](
        handle_request: Callable[Concatenate[Request, P], Awaitable[Response]],
    ):
        async def wrapper(request: Request, *args: P.args, **kwargs: P.kwargs):
...
def require_permission(
    ...
    def has_permission_decorator[**P](
        handle_request: Callable[Concatenate[Request, P], Awaitable[Response]],
    ):
        async def wrapper(request: Request, *args: P.args, **kwargs: P.kwargs):
            user, session_id = await get_user(request, session_only)
            ...
            user_has_permission = await handle(
                CheckUserHasPermissionCommand(user.id, permission_name, check_denied_only)
            )
            if user_has_permission:
                ...
                raise Problem(
                    ...
                    f"User does not have required permission '{permission_name}'",
                    status=401,
                )
...
def check_series_visibility[**P](
    ...
    async def wrapper(request: Request, *args: P.args, **kwargs: P.kwargs):
        ...
        if user_has_permission:
            ...
            raise Problem(
                "Insufficient permission",
                f"User does not have required permission '{series_permissions.VIEW_HIDDEN_SERIES}'",
                status=401,
            )
...
def check_tournament_visiblity[**P](
    handle_request: Callable[Concatenate[Request, P], Awaitable[Response]],
):
    async def wrapper(request: Request, *args: P.args, **kwargs: P.kwargs):
        tournament_id = request.path_params.get("tournament_id", None)
        ...
        if user_has_permission:
            ...
            raise Problem(
                "Insufficient permission",
                f"User does not have required permission '{tournament_permissions.VIEW_HIDDEN_TOURNAMENT}'",
                status=401,
            )

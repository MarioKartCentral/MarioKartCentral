...
class CreateTournamentCommand(Command[None]):
    ...
    async def handle(self, db_wrapper, s3_wrapper):
        ...
        await s3_wrapper.put_object(s3.TOURNAMENTS, f'{tournament_id}.json', s3_message)
...
class EditTournamentCommand(Command[None]):
    ...
    async def handle(self, db_wrapper, s3_wrapper):
        ...
        async with db_wrapper.connect() as db:
            ...
            s3_data = await s3_wrapper.get_object(s3.TOURNAMENTS, f'{self.id}.json')
            if s3_data is None:
                raise Problem("No tournament found", status=404)
            ...
            await s3_wrapper.put_object(s3.TOURNAMENTS, f'{self.id}.json', s3_message)
...
class GetTournamentDataCommand(Command[GetTournamentRequestData]):
    ...
    async def handle(self, db_wrapper, s3_wrapper):
        body = await s3_wrapper.get_object(s3.TOURNAMENTS, f'{self.id}.json')
        if body is None:
            raise Problem('No tournament found', status=404)

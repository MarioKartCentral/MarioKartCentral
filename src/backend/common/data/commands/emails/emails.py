    ...
    async def handle(self, db_wrapper: DBWrapper, email_service: EmailService):
        async with db_wrapper.connect(db_name="auth") as db:
            async with db.execute(
                "SELECT email, email_confirmed FROM user_auth WHERE user_id = :user_id",
            ...
            await db.execute(
                """
                INSERT INTO email_verifications(token_id, user_id, expires_on)
                VALUES(:token_id, :user_id, :expires_on)
                """,
                {
                    "token_id": token_id,
                    "user_id": self.user_id,
...
class ChangeEmailCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(db_name="auth") as db:
            ...
            async with db.execute(
                "SELECT email FROM user_auth WHERE email = :new_email AND user_id != :user_id",
                {"new_email": self.new_email, "user_id": self.user_id},
            ) as cursor:
                row = await cursor.fetchone()
                if row:
                    raise Problem("Email is used by another user", status=400)
            await db.execute(
                "UPDATE user_auth SET email = :new_email, email_confirmed = 0 WHERE user_id = :user_id",
                {"new_email": self.new_email, "user_id": self.user_id},
            )
            await db.commit()
...
class VerifyEmailCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(db_name="auth") as db:
            async with db.execute(
                "SELECT user_id FROM email_verifications WHERE token_id = :token_id",
                ...
                user_id = row[0]
            await db.execute(
                "DELETE FROM email_verifications WHERE token_id = :token_id",
                {"token_id": self.token_id},
            )
            await db.execute(
                "UPDATE user_auth SET email_confirmed = 1 WHERE user_id = :user_id",
                {"user_id": user_id},
            )
            await db.commit()
...
class SendPasswordResetEmailCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper, email_service: EmailService):
        async with db_wrapper.connect(db_name="auth") as db:
            async with db.execute(
                "SELECT user_id FROM user_auth WHERE email = :user_email",
            ...
            expires_on = int((datetime.now(timezone.utc) + timedelta(minutes=60)).timestamp())
            await db.execute(
                "INSERT INTO password_resets(token_id, user_id, expires_on) VALUES(:token_id, :user_id, :expires_on)",
                {
                    "token_id": token_id,
                    "user_id": user_id,
                    "expires_on": expires_on,
        ...
        content = f"""You requested a password reset on MKCentral.
        You can reset your password by clicking the link below (expires in 60 minutes): \
...
class ResetPasswordWithTokenCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(db_name="auth") as db:
            async with db.execute(
                "SELECT user_id FROM password_resets WHERE token_id = :token_id",
                {"token_id": self.token_id},
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
                ...
                user_id = row[0]
            await db.execute(
                "DELETE FROM password_resets WHERE token_id = :token_id",
                {"token_id": self.token_id},
            )
            await db.execute(
                """
                UPDATE user_auth
                SET password_hash = :new_password_hash, force_password_reset = 0
...
class ResetPasswordCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(db_name="auth") as db:
            ...
            await db.execute(
                """
                UPDATE user_auth SET password_hash = :new_password_hash, force_password_reset = 0
                WHERE user_id = :user_id
                """,
                {
                    "new_password_hash": self.new_password_hash,
...
class RemoveExpiredTokensCommand(Command[None]):
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(db_name="auth") as db:
            ...
            await db.execute(
                "DELETE FROM email_verifications WHERE expires_on < :now",
                {"now": now},
            )
            await db.execute(
                "DELETE FROM password_resets WHERE expires_on < :now",
                {"now": now},
            )
            await db.commit()

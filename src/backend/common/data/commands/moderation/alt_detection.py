...
class ListAltFlagsCommand(Command[AltFlagList]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(
            db_name="main",
            attach=["user_activity", "alt_flags"],
            readonly=True,
        ) as db:
            limit = 20
            offset = 0
            ...
            count_query = """
                SELECT COUNT(*) FROM alt_flags.alt_flags
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(
            ...
            async with db.execute(
                ...
                {
                    "user_id": user_id,
                    "exclude_fingerprints": self.exclude_fingerprints,
                },
            ) as cursor:
                rows = await cursor.fetchall()
                for row in rows:
...
class ViewPlayerLoginHistoryCommand(Command[PlayerUserLogins]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(db_name="main", attach=["user_activity"]) as db:
            async with db.execute(
                """
                SELECT ul.id, ul.user_id, ul.ip_address_id,
...
class ViewPlayerIPHistoryCommand(Command[PlayerIPHistory]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(db_name="main", attach=["user_activity"]) as db:
            async with db.execute(
                """
                SELECT tr.id, tr.date_earliest, tr.date_latest, tr.times,
...
class ViewHistoryForIPCommand(Command[IPHistory]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(db_name="main", attach=["user_activity"]) as db:
            async with db.execute(
                """
                SELECT ip.id, ip.ip_address, ip.is_mobile, ip.is_vpn,
                    ip.country, ip.region, ip.city, ip.asn,
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(db_name="user_activity") as db:
            query = """
                ...
                WHERE (:ip_address IS NULL OR ip.ip_address LIKE :ip_address)
                    AND (:city IS NULL OR ip.city LIKE :city)
            ...
            async with db.execute(
                f"""
                SELECT ip.id, ip.ip_address, ip.is_mobile, ip.is_vpn,
                    ip.country, ip.city, ip.region, ip.asn, COUNT(uip.user_id)

...
class BanPlayerCommand(Command[PlayerBan]):
    ...
    async def player_exists_in_table(
        self,
        db: Connection,
        table: Literal["players", "player_bans"],
        player_id: int,
    ) -> bool:
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            command = """
            INSERT INTO player_bans(player_id, banned_by, is_indefinite, ban_date, expiration_date, reason, comment)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """
            params = (
                self.player_id,
            ...
            async with db.execute(
                """UPDATE players SET is_banned = TRUE WHERE id = ?""",
                (self.player_id,),
            ) as cursor:
...
class UnbanPlayerCommand(Command[PlayerBanHistorical]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                """
                SELECT player_id, banned_by, is_indefinite, ban_date, expiration_date, reason, comment
                FROM player_bans
                WHERE player_id = ?
                """,
                (self.player_id,),
                ...
                if row is None:
                    raise Problem(
                        "Player not found",
                        "Unable to find player in the ban table",
                        status=404,
                    )
            (
                player_id,
            ...
            async with db.execute(
                """
                INSERT INTO player_bans_historical(
                    player_id, banned_by, unbanned_by, unban_date, is_indefinite, ban_date,
                    expiration_date, reason, comment)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    player_id,
            ...
            async with db.execute(
                """UPDATE players SET is_banned = FALSE WHERE id = ?""",
                (self.player_id,),
            ) as cursor:
                if cursor.rowcount != 1:
                    raise Problem(
...
class EditPlayerBanCommand(Command[PlayerBan]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            command = """
            UPDATE player_bans
            SET banned_by = ?, is_indefinite = ?, expiration_date = ?, reason = ? , comment = ?
            WHERE player_id = ?
            """
            params = (
                self.banned_by,
            ...
            async with db.execute(
                "SELECT ban_date FROM player_bans WHERE player_id = ?",
                (self.player_id,),
            ) as cursor:
                row = await cursor.fetchone()
                assert row is not None
...
def _append_equal_filter(
    where_clauses: list[str],
    variable_parameters: list[Any],
    filter_value: Any,
    where_clause: str,
    var_param: str | None = None,
):
...
class ListBannedPlayersCommand(Command[PlayerBanList]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(readonly=True) as db:
            ...
            ban_query = f"""
                SELECT p.name, p.id, p.country_code, b.is_indefinite, b.ban_date,
                    b.expiration_date, b.reason, b.comment, b.banned_by, bbp.id, bbp.name
                FROM {joined_tables}
                {where_clause}
            ...
            async with db.execute(
                f"SELECT COUNT (*) FROM {joined_tables} {where_clause}",
                variable_parameters,
            ) as cursor:
                row = await cursor.fetchone()
                assert row is not None
        ...
        async with db_wrapper.connect(readonly=True) as db:
            ...
            if filter.unbanned_by and filter.unbanned_by.lower() in "system":
                _append_equal_filter(
                    ...
                    variable_parameters,
                    filter.unbanned_by,
                    "(ubp.name LIKE ? OR ubu.id IS NULL)",
                    f"{filter.unbanned_by}%",
                )
            else:
                _append_equal_filter(
                    where_clauses,
            ...
            _append_equal_filter(
                where_clauses,
                variable_parameters,
                filter.unbanned_before,
                "b.unban_date <= ?",
            )
            _append_equal_filter(
                where_clauses,
                variable_parameters,
                filter.unbanned_after,
                "b.unban_date >= ?",
            )
            ...
            # bbu: BannedByUser, bbp: BannedByPlayer
            ban_query = f"""
            SELECT p.name, p.id, p.country_code, b.is_indefinite, b.ban_date, b.expiration_date
                b.reason, b.comment, b.banned_by, bbp.id, bbp.name, b.unban_date, b.unbanned_by, ubp.id, ubp.name
            FROM {joined_tables}
            {where_clause}
            ORDER BY b.ban_date DESC
            LIMIT ? OFFSET ?
...
class GetPlayersToUnbanCommand(Command[list[PlayerBanWithUserId]]):
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(readonly=True) as db:
            ...
            query = """
                SELECT p.player_id, p.banned_by, p.is_indefinite, p.ban_date, p.expiration_date,
                    p.reason, p.comment, u.id
                FROM player_bans p 

...
class BanPlayerCommand(Command[PlayerBan]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            ...
            params = (self.player_id, self.banned_by, data.is_indefinite,
                      ban_date, data.expiration_date, data.reason, data.comment)

            try:
                player_row = await db.execute_insert(command, params)
                if player_row is None:
                    raise Problem("Failed to ban player",
                                  "Failed to insert into ban table")
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            ...
            async with db.execute("SELECT player_id, banned_by, is_indefinite, ban_date, expiration_date, reason, comment FROM player_bans WHERE player_id = ?", (self.player_id,)) as cursor:
                ...
                if row is None:
                    raise Problem(
                        "Player not found", "Unable to find player in the ban table", status=404)
            ...
            async with db.execute("INSERT INTO player_bans_historical(player_id, banned_by, unbanned_by, unban_date, is_indefinite, ban_date, expiration_date, reason, comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (player_id, banned_by, self.unbanned_by, unban_date, is_indefinite, ban_date, expiration_date, reason, comment)) as cursor:
                ...
                if rowcount != 1:
                    raise Problem("Failed to unban player",
                                  "Failed to insert into historical ban table")
            ...
            async with db.execute("""UPDATE players SET is_banned = FALSE WHERE id = ?""", (self.player_id,)) as cursor:
                if cursor.rowcount != 1:
                    raise Problem("Failed to unban player",
                                  "Failed to update is_banned in player table")
            await db.commit()
...
class EditPlayerBanCommand(Command[PlayerBan]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            ...
            params = (self.banned_by, data.is_indefinite, data.expiration_date,
                      data.reason, data.comment, self.player_id)
...
class ListBannedPlayersCommand(Command[PlayerBanList]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(readonly=True) as db:
            ...
            _append_equal_filter(
                where_clauses, variable_parameters, filter.player_id, 'p.id = ?')
            _append_equal_filter(where_clauses, variable_parameters,
                                 filter.name, 'p.name LIKE ?', f"{filter.name}%")
            _append_equal_filter(where_clauses, variable_parameters,
                                 filter.banned_by, 'bbp.name LIKE ?', f"{filter.reason}%")
            _append_equal_filter(
                where_clauses, variable_parameters, filter.is_indefinite, 'b.is_indefinite = ?')
            _append_equal_filter(where_clauses, variable_parameters,
                                 filter.expires_before, 'b.expiration_date <= ?')
            _append_equal_filter(where_clauses, variable_parameters,
                                 filter.expires_after, 'b.expiration_date >= ?')
            _append_equal_filter(
                where_clauses, variable_parameters, filter.banned_before, 'b.ban_date <= ?')
            _append_equal_filter(
                where_clauses, variable_parameters, filter.banned_after, 'b.ban_date >= ?')
            _append_equal_filter(where_clauses, variable_parameters,
                                 filter.reason, "b.reason LIKE ?", f"%{filter.reason}%")

            if filter.comment and filter.comment.strip():
                _append_equal_filter(where_clauses, variable_parameters,
                                     filter.comment, "b.comment LIKE ?", f"%{filter.comment}%")
            ...
            page_count = int(ban_count / limit) + \
                (1 if ban_count % limit else 0)
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(readonly=True) as db:
            ...
            _append_equal_filter(where_clauses, variable_parameters,
                                 filter.name, 'p.name LIKE ?', f"{filter.name}%")
            _append_equal_filter(where_clauses, variable_parameters,
                                 filter.banned_by, 'bbp.name LIKE ?', f"{filter.banned_by}%")
            _append_equal_filter(
                where_clauses, variable_parameters, filter.is_indefinite, 'b.is_indefinite = ?')
            _append_equal_filter(where_clauses, variable_parameters,
                                 filter.expires_before, 'b.expiration_date <= ?')
            ...
            if filter.unbanned_by and filter.unbanned_by.lower() in 'system':
                _append_equal_filter(where_clauses, variable_parameters, filter.unbanned_by,
                                     '(ubp.name LIKE ? OR ubu.id IS NULL)', f"{filter.unbanned_by}%")
            else:
                _append_equal_filter(where_clauses, variable_parameters,
                                     filter.unbanned_by, 'ubp.name LIKE ?', f"{filter.unbanned_by}%")
            _append_equal_filter(
                where_clauses, variable_parameters, filter.unbanned_before, 'b.unban_date <= ?')
            _append_equal_filter(
                where_clauses, variable_parameters, filter.unbanned_after, 'b.unban_date >= ?')

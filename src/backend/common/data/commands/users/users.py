    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(db_name="main", attach=["auth"]) as db:
            row = await db.execute_insert(
                "INSERT INTO users(join_date) VALUES(:join_date)",
                {"join_date": now},
            )
    ...
    async def handle(self, db_wrapper: DBWrapper) -> int | None:
        async with db_wrapper.connect(db_name="main", readonly=True) as db:
            async with db.execute(
                "SELECT player_id FROM users WHERE id = :user_id",
...
class GetInvitesForPlayerCommand(Command[PlayerInvites]):
    ...
    async def handle(self, db_wrapper: DBWrapper) -> PlayerInvites:
        ...
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                """
                SELECT i.id, i.date, i.is_bagger_clause, t.id, t.name, t.tag,
                    t.color, r.id, r.name, r.tag, r.game, r.mode, r.color
            ...
            async with db.execute(
                """
                SELECT i.id, i.tournament_id, i.timestamp, i.is_bagger_clause, s.name, s.tag, s.color,
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(db_name="main", attach=["auth"], readonly=True) as db:
            ...
            query = """
                WITH filtered_users AS (
                    SELECT u.id, u.player_id, u.join_date,
                        ...
                        p.name, p.country_code, p.is_hidden, p.is_shadow, p.is_banned,
                        p.join_date as player_join_date
                    FROM users u
                    JOIN auth.user_auth a ON u.id = a.user_id
                    LEFT JOIN players p ON u.player_id = p.id
                    WHERE (
                        :name_or_email IS NULL
                        OR a.email LIKE '%' || :name_or_email || '%'
                        OR p.name LIKE '%' || :name_or_email || '%'
                    )
                )
            ...
            async with db.execute(
                query,
                {
                    "name_or_email": self.filter.name_or_email,
                    "limit": limit,
                    "offset": offset,
                },
            ) as cursor:
                async for row in cursor:
                    ...
                    users.append(
                        UserInfo(
                            ...
                            email,
                            join_date,
                            bool(email_confirmed_int),
                            bool(force_password_reset_int),
                            player,
                        )
                    )
...
class ViewUserCommand(Command[UserInfoDetailed]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(db_name="main", attach=["auth"], readonly=True) as db:
            ...
            # admins should only be able to view API tokens for users with
            # lower permissions than themselves (or themselves)
            is_privileged = True
            if self.mod_user_id != self.user_id:
                perm_query = """
                    ...
                    SELECT EXISTS(
                        SELECT 1 FROM mh
                        JOIN uh
                        WHERE uh.pos IS NULL
                            OR mh.pos < uh.pos
                    )
                """
                async with db.execute(
                    perm_query,
                    {"mod_user_id": self.mod_user_id, "user_id": self.user_id},
                ) as cursor:
                    row = await cursor.fetchone()
            ...
            if is_privileged:
                async with db.execute(
                    "SELECT token_id, name FROM auth.api_tokens WHERE user_id = ?",
                    (self.user_id,),
                ) as cursor:
                    rows = await cursor.fetchall()
        ...
        async with db_wrapper.connect(db_name="main", attach=["auth"]) as db:
            ...
            if self.mod_user_id != self.user_id:
                ...
                async with db.execute(
                    ...
                    if not row or not row[0]:
                        raise Problem(
                            "Cannot edit users higher/equal in the role hierarchy to yourself",
                            status=403,
                        )

    ...
    async def handle(self, db_wrapper: DBWrapper) -> str:
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute("SELECT name FROM players WHERE id = ?", (self.player_id,)) as cursor:
                ...
                if row is None:
                    raise Problem(
                        "Dispatching notification failed to query player name",
                        status=500,
                    )
                return row[0]
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute("SELECT name FROM teams WHERE id = ?", (self.team_id,)) as cursor:
                ...
                if row is None:
                    raise Problem(
                        "Dispatching notification failed to query team",
                        status=500,
                    )
                return row[0]
...
class GetSeriesNameFromIdCommand(Command[str]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                "SELECT name FROM tournament_series WHERE id = ?",
                (self.series_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if row is None:
                    raise Problem(
                        "Dispatching notification failed to query series",
                        status=500,
                    )
                return row[0]
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                ...
                if row is None:
                    raise Problem(
                        "Dispatching notification failed to query tournament",
                        status=500,
                    )
                return row[0]
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute("SELECT type FROM friend_codes WHERE id = ?", (self.fc_id,)) as cursor:
                ...
                if row is None:
                    raise Problem(
                        "Dispatching notification failed to query fc type",
                        status=500,
                    )
                return row[0]
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            ...
            async with db.execute(query, (self.request_id,)) as cursor:
                ...
                if row is None:
                    raise Problem(
                        "Dispatching notification failed to query name edit data",
                        status=500,
                    )
...
class GetNotificationDataFromTeamTransfersCommand(Command[NotificationDataTeamTransfer]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            query = """
                SELECT p.name, p.id, t.id, t.name, r.name
                FROM players p 
            ...
            async with db.execute(query, (self.invite_id,)) as cursor:
                ...
                if row is None:
                    raise Problem(
                        "Dispatching notification failed to query team transfer data",
                        status=500,
                    )
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            ...
            async with db.execute(query, (self.edit_request_id,)) as cursor:
                ...
                if row is None:
                    raise Problem(
                        "Dispatching notification failed to query edit data",
                        status=500,
                    )
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            ...
            async with db.execute(query, (self.roster_id,)) as cursor:
                ...
                if row is None:
                    raise Problem(
                        "Dispatching notification failed to query roster data",
                        status=500,
                    )

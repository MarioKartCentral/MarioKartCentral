    ...
    async def handle(self, db_wrapper: DBWrapper) -> int:
        ...
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                """
                SELECT is_squad, registrations_open, squad_tag_required, squad_name_required, mii_name_required,
                    teams_only, require_single_fc, bagger_clause_enabled, checkins_open
                FROM tournaments
                WHERE ID = ?
                """,
                (self.tournament_id,),
                ...
                if not bool(squad_tag_required) and self.squad_tag is not None:
                    raise Problem(
                        "Tournament does not accept squad tags, please set this value to null",
                        status=400,
                    )
                ...
                if not bool(squad_name_required) and self.squad_name is not None:
                    raise Problem(
                        "Tournament does not accept squad names, please set this value to null",
                        status=400,
                    )
                ...
                if not bool(mii_name_required) and self.mii_name is not None:
                    raise Problem(
                        "Tournament does not accept Mii Names, please set this value to null",
                        status=400,
                    )
                if bool(teams_only):
                    raise Problem(
                        "This tournament is teams only, cannot create new squads",
                        status=400,
                    )
                ...
                if require_single_fc and not self.selected_fc_id:
                    raise Problem(
                        "Please select an FC to use for this tournament",
                        status=400,
                    )
                if not bagger_clause_enabled and self.is_bagger_clause:
                    raise Problem(
                        "Cannot register as bagger when bagger clause is not enabled",
                        status=400,
                    )
            ...
            if existing_registration_id is not None:
                ...
                async with db.execute(
                    "SELECT is_registered FROM tournament_registrations WHERE id IS ?",
            ...
            async with db.execute(
                """INSERT INTO tournament_registrations(
                    name, tag, color, timestamp, tournament_id, is_registered, is_approved)
                VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (
                    self.squad_name,
                    self.squad_tag,
            ...
            await db.execute(
                """INSERT INTO tournament_players(
                    player_id, tournament_id, registration_id, is_squad_captain, timestamp, is_checked_in, mii_name,
                    can_host, is_invite, selected_fc_id, is_representative, is_bagger_clause, is_approved)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    self.captain_player_id,
                    self.tournament_id,
...
class RegisterTeamTournamentCommand(Command[int]):
    ...
    async def handle(self, db_wrapper: DBWrapper) -> int:
        ...
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                """
                SELECT registrations_open, teams_allowed, min_representatives, min_squad_size, max_squad_size,
                    bagger_clause_enabled, game, team_members_only
                FROM tournaments
                WHERE ID = ?
                """,
                (self.tournament_id,),
                ...
                if len(self.players) and len(captains) != 1:
                    raise Problem(
                        f"""Exactly one player must be selected as a captain,
                        but you have selected {len(captains)} captains""",
                        status=400,
                    )
                ...
                if min_representatives and len(representatives) < min_representatives:
                    raise Problem(
                        f"Must have at least {min_representatives} representatives for this tournament",
                        status=400,
                    )
                baggers = [p.player_id for p in self.players if p.is_bagger_clause]
                if len(baggers) and not bool(bagger_clause_enabled):
                    raise Problem(
                        "Cannot register players as baggers when bagger clause is not enabled",
                        status=400,
                    )
                if team_members_only and min_squad_size and len(self.players) < min_squad_size:
                    raise Problem(
                        f"Number of players is less than this tournament's minimum squad size ({min_squad_size})",
                        status=400,
                    )
                if max_squad_size and len(self.players) > max_squad_size:
                    raise Problem(
                        f"Number of players is greater than this tournament's maximum squad size({max_squad_size})",
                        status=400,
                    )
            ...
            for roster_id in self.roster_ids:
                async with db.execute(
                    "SELECT id, game FROM team_rosters WHERE id = ?",
                    (roster_id,),
                ) as cursor:
                    row = await cursor.fetchone()
                    if not row:
                        raise Problem(
                            f"Roster with ID {roster_id} does not exist",
                            status=400,
                        )
                    found_roster_id, found_roster_game = row
                    if found_roster_game != game:
                        raise Problem(
                            f"""Roster with ID {found_roster_id} is for the game {found_roster_game},
                            but this tournament is for {game}""",
                            status=400,
                        )
            ...
            async with db.execute(already_registered_query, (self.tournament_id,)) as cursor:
                ...
                if len(already_registered_names):
                    raise Problem(
                        f"""The following players are already registered for this tournament:
                        {", ".join(already_registered_names)}""",
                        status=400,
                    )
            ...
            async with db.execute(team_player_query) as cursor:
                ...
                if len(invalid_players):
                    ...
                    raise Problem(
                        f"""The following player IDs are not registered for any of the selected rosters:
                        {", ".join(player_ids)}""",
                        status=400,
                    )
            ...
            await db.executemany(
                "INSERT INTO team_squad_registrations(roster_id, registration_id, tournament_id) VALUES (?, ?, ?)",
                team_squad_rows,
            )
...
class EditSquadCommand(Command[TournamentSquadUpdate]):
    ...
    async def handle(self, db_wrapper: DBWrapper) -> TournamentSquadUpdate:
        async with db_wrapper.connect() as db:
            async with db.execute(
                """
                SELECT id, is_approved, name, tag, color, is_registered
                FROM tournament_registrations
                WHERE id = ? AND
                    tournament_id = ?
                """,
                (self.registration_id, self.tournament_id),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
            ...
            async with db.execute(
                ...
                SET name = ?, tag = ?, color = ?, is_registered = ?, is_approved = ?
            ...
            if not is_registered:
                await db.execute(
                    "DELETE FROM tournament_players WHERE registration_id = ? AND tournament_id = ?",
                    (self.registration_id, self.tournament_id),
                )
            await db.commit()
...
class ChangeSquadCaptainCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            ...
            await db.execute(
                "UPDATE tournament_players SET is_squad_captain = ? WHERE tournament_id = ? AND registration_id = ?",
                (False, self.tournament_id, self.registration_id),
            )
            await db.execute(
                """
                UPDATE tournament_players
                SET is_squad_captain = ?
                WHERE tournament_id = ?
                    AND registration_id = ?
                    AND player_id = ?
                """,
...
class AddRepresentativeCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT max_representatives FROM tournaments WHERE id = ?",
                (self.tournament_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
            ...
            async with db.execute(
                ...
                """,
                (self.tournament_id, self.registration_id),
            ) as cursor:
                row = await cursor.fetchone()
                assert row is not None
                rep_count: int = row[0]
                if max_representatives and rep_count >= max_representatives:
                    raise Problem(
                        f"This tournament has a maximum of {max_representatives} representatives",
                        status=400,
                    )
            async with db.execute(
                """
...
class UnregisterSquadCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            ...
            await db.execute(
                "DELETE FROM team_squad_registrations WHERE registration_id = ? AND tournament_id = ?",
                (self.registration_id, self.tournament_id),
            )
            await db.execute(
                "DELETE FROM tournament_placements WHERE registration_id = ?",
                (self.registration_id,),
            )
            await db.execute(
...
class GetSquadDetailsCommand(Command[TournamentSquadDetails]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                """
                SELECT id, name, tag, color, timestamp, is_registered, is_approved
                FROM tournament_registrations
                WHERE tournament_id = ?
                    AND id = ?
                """,
            ...
            async with db.execute(
                """
                SELECT tsr.registration_id, tr.id, tr.team_id, tr.name, tr.tag, tr.color, t.name, t.tag, t.color
                FROM team_squad_registrations tsr
                JOIN team_rosters tr ON tsr.roster_id = tr.id
                JOIN teams t ON tr.team_id = t.id
                WHERE tsr.registration_id = ?
                """,
                (self.registration_id,),
            ) as cursor:
                rows = await cursor.fetchall()
                for row in rows:
                    ...
                    roster = RosterBasic(
                        ...
                        team_name,
                        team_tag,
                        roster_color if roster_color else team_color,
                        roster_id,
                        roster_name if roster_name else team_name,
                        roster_tag if roster_tag else team_tag,
                    )
                    rosters.append(roster)

            async with db.execute(
                """
                SELECT t.id, t.player_id, t.is_squad_captain, t.is_representative, t.timestamp, t.is_checked_in, 
                    t.mii_name, t.can_host, t.is_invite, t.selected_fc_id, t.is_bagger_clause,
                    t.is_approved, t.is_eligible, p.name, p.country_code, p.is_banned, d.discord_id, d.username,
                    d.discriminator, d.global_name, d.avatar
                ...
                player_dict: dict[
                    int, SquadPlayerDetails
                ] = {}  # creating a dictionary of players so we can add their FCs to them later
                fc_id_list: list[
                    int
                ] = []  # if require_single_fc is true, we will need to know exactly which FCs to retrieve
                for row in player_rows:
                    ...
                    curr_player = SquadPlayerDetails(
                        reg_id,
                        player_id,
                        self.registration_id,
                        player_timestamp,
                    ...
                    # Add this FC's ID to the list of FCs we query for if require_single_fc
                    # is true for the current tournament
                    player_dict[curr_player.player_id] = curr_player
                    fc_id_list.append(curr_fc_id)

            # check if only single FCs are allowed or not
            async with db.execute(
                "SELECT require_single_fc FROM tournaments WHERE id = ?",
                (self.tournament_id,),
            ) as cursor:
                row = await cursor.fetchone()
                assert row is not None
                ...
                if require_single_fc:
                    # convert all FC IDs to str and join with a comma
                    fc_where_clause = f" AND id IN ({','.join(map(str, fc_id_list))})"
            # gathering all the valid FCs for each player for this tournament
            fc_query = f"""
                SELECT player_id, fc
                FROM friend_codes
                WHERE player_id IN ({",".join(map(str, player_dict.keys()))})
                {fc_where_clause}
            """
...
class AddRosterToSquadCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT game, mode, teams_allowed, registrations_open FROM tournaments WHERE id = ?",
                (self.tournament_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
            ...
            if not teams_allowed:
                raise Problem(
                    "Cannot add rosters to a squad if teams are not allowed",
                    status=400,
                )
            ...
            async with db.execute(
                ...
                if not row:
                    raise Problem("Squad not found", status=404)
            async with db.execute(
                "SELECT game, mode FROM team_rosters WHERE id = ?",
                (self.roster_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
                ...
                if game != roster_game or mode != roster_mode:
                    raise Problem(
                        "The tournament's game/mode and the roster's game/mode do not match",
                        status=400,
                    )
            ...
            if not self.is_privileged:
                async with db.execute(
                    ...
                    if not row:
                        raise Problem(
                            "You do not have permissions to register this roster for a tournament",
                            status=400,
                        )

            async with db.execute(
                "SELECT registration_id FROM team_squad_registrations WHERE tournament_id = ? AND roster_id = ?",
                (self.tournament_id, self.roster_id),
            ) as cursor:
                row = await cursor.fetchone()
                if row:
                    raise Problem(
                        "This roster is already registered for this tournament",
                        status=400,
                    )
            ...
            async with db.execute(
                ...
                """,
                (self.roster_id, self.tournament_id, False),
            ) as cursor:
                rows = await cursor.fetchall()
                new_players = [
                    (
            ...
            await db.execute(
                "INSERT INTO team_squad_registrations(roster_id, registration_id, tournament_id) VALUES(?, ?, ?)",
                (self.roster_id, self.registration_id, self.tournament_id),
            )
            await db.executemany(
                """
                INSERT INTO tournament_players(
                    player_id, tournament_id, registration_id, is_squad_captain, timestamp, is_checked_in,
...
class RemoveRosterFromSquadCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT registrations_open, teams_only FROM tournaments WHERE id = ?",
                (self.tournament_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
            ...
            async with db.execute("SELECT id FROM team_rosters WHERE id = ?", (self.roster_id,)) as cursor:
                ...
                if not row:
                    raise Problem("Team roster not found", status=404)
            async with db.execute(
                "SELECT registration_id FROM team_squad_registrations WHERE registration_id = ? AND roster_id = ?",
                (self.registration_id, self.roster_id),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
            ...
            if not self.is_privileged:
                async with db.execute(
                    ...
                    if row:
                        raise Problem(
                            """You cannot remove this roster from this squad as it would
                            unregister yourself from the tournament.""",
                            status=400,
                        )

            await db.execute(
                """
                DELETE FROM team_squad_registrations
                WHERE roster_id = ?
                    AND registration_id = ?
                    AND tournament_id = ?
                """,
            ...
            if teams_only:
                await db.execute(
                    """DELETE FROM tournament_players WHERE registration_id = ? AND tournament_id = ? AND player_id IN (

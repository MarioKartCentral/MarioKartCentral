    ...
    async def handle(self, db_wrapper: DBWrapper) -> SquadPlayerDetails:
        ...
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                """
                SELECT game, is_squad, max_squad_size, mii_name_required, registrations_open, team_members_only,
                    require_single_fc, bagger_clause_enabled, checkins_open
                FROM tournaments
                WHERE id = ?
                """,
                (self.tournament_id,),
                ...
                if bool(is_squad) and self.registration_id is None:
                    raise Problem(
                        "Players may not register alone for squad tournaments",
                        status=400,
                    )
                if not bool(is_squad) and self.registration_id is not None:
                    raise Problem(
                        "Players may not register for a squad for solo tournaments",
                        status=400,
                    )
                ...
                if not is_squad and (self.is_squad_captain or self.is_representative):
                    raise Problem(
                        "is_squad_captain and is_representative fields must be false when is_squad is true",
                        status=400,
                    )
            ...
            async with db.execute(
                "SELECT name, country_code, is_banned FROM players WHERE id = ?",
                (self.player_id,),
            ) as cursor:
            ...
            if registration_id is not None:
                async with db.execute(
                    "SELECT tag FROM tournament_registrations WHERE id = ? AND tournament_id = ?",
                    (registration_id, self.tournament_id),
                ) as cursor:
                    row = await cursor.fetchone()
                    if row is None:
                ...
                async with db.execute(
                    ...
                    INSERT INTO tournament_registrations(
                        color, timestamp, tournament_id, is_registered, is_approved)
                    VALUES(?, ?, ?, ?, ?)
                    """,
                    (0, timestamp, self.tournament_id, True, self.is_approved),
                ) as cursor:
            ...
            if bool(team_members_only):
                async with db.execute(
                    ...
                    WHERE m.player_id = ?
                        AND m.leave_date IS ?
                        AND r.registration_id IS ?
                    """,
                    (self.player_id, None, self.registration_id),
                ) as cursor:
                    row = await cursor.fetchone()
                    if row is None:
                        raise Problem(
                            "Player must be registered for a team roster linked to this squad",
                            status=400,
                        )
            ...
            async with db.execute(
                """
                SELECT registration_id, is_bagger_clause
                ...
                for row in rows:
                    ...
                    if existing_registration_id == self.registration_id:
                        raise Problem(
                            "Player is already invited to/registered for this squad",
                            status=400,
                        )
            ...
            if self.registration_id is not None and max_squad_size is not None:
                async with db.execute(
                    "SELECT id FROM tournament_players WHERE tournament_id = ? AND registration_id IS ?",
            ...
            async with db.execute(
                ...
                """,
                (self.player_id, game_fc_map[game]),
            ) as cursor:
                rows = await cursor.fetchall()
...
class EditPlayerRegistrationCommand(Command[PlayerRegistrationUpdate]):
    ...
    async def handle(self, db_wrapper: DBWrapper) -> PlayerRegistrationUpdate:
        async with db_wrapper.connect() as db:
            async with db.execute(
                """
                SELECT is_squad, mii_name_required, registrations_open,
                require_single_fc, bagger_clause_enabled, checkins_open
                FROM tournaments
                WHERE id = ?
                """,
                ...
                if (not self.is_privileged) and (not registrations_open):
                    raise Problem(
                        "Registrations are closed, so you cannot edit your registration details",
                        status=400,
                    )
            ...
            async with db.execute(
                """
                SELECT id, is_invite, is_representative, is_squad_captain, is_checked_in, is_bagger_clause, is_approved
                FROM tournament_players
                WHERE tournament_id = ? 
            ...
            # if a player accepts an invite while already registered for a different squad,
            # their old registration must be removed
            if curr_is_invite and (not self.is_invite):
                old_registration_id = None
                old_is_squad_captain = False
                async with db.execute(
                    """
                    SELECT registration_id, is_squad_captain
                    FROM tournament_players
                    WHERE tournament_id = ?
                        AND player_id = ?
                        AND is_invite = ?
                ...
                if old_registration_id:
                    ...
                    async with db.execute(
                        ...
                        WHERE tournament_id = ?
                            ...
                            AND is_invite = 0
                        """,
                        (self.tournament_id, old_registration_id),
                    ) as cursor:
                        row = await cursor.fetchone()
                    ...
                    if old_is_squad_captain and num_squad_players > 1:
                        raise Problem(
                            """Please unregister your current squad or give captain to another
                            player in your squad before leaving your current squad""",
                            status=400,
                        )
                    if num_squad_players == 1:
                        ...
                        await db.execute(
                            "DELETE FROM team_squad_registrations WHERE registration_id = ?",
                            (self.registration_id,),
                        )
                        await db.execute(
            ...
            if self.is_approved is not None and not is_squad:
                await db.execute(
                    "UPDATE tournament_registrations SET is_approved = ? WHERE id = ?",
                    (self.is_approved, self.registration_id),
                )
...
class UnregisterPlayerCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT registrations_open FROM tournaments WHERE id = ?",
                (self.tournament_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if row is None:
                ...
                if (not self.is_privileged) and (not registrations_open):
                    raise Problem(
                        "Registrations are closed, so players cannot be unregistered from this tournament",
                        status=400,
                    )
            async with db.execute(
                """
                SELECT is_squad_captain, is_invite
                FROM tournament_players
                WHERE tournament_id = ?
                    AND registration_id is ?
                    AND player_id = ?
            ...
            if is_squad_captain and num_squad_players > 1:
                raise Problem(
                    """Please unregister your current squad or give captain to
                    another player in your squad before unregistering for this tournament""",
                    status=400,
                )

            async with db.execute(
                "DELETE FROM tournament_players WHERE tournament_id = ? AND registration_id IS ? AND player_id = ?",
                (self.tournament_id, self.registration_id, self.player_id),
            ) as cursor:
...
class GetTournamentRegistrationsCommand(Command[list[TournamentSquadDetails]]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                """
                SELECT is_squad, game, verification_required, checkins_enabled, min_players_checkin,
                    min_squad_size, teams_allowed
                FROM tournaments
                WHERE id = ?
                """,
                (self.tournament_id,),
                ...
                if min_squad_size:
                    where_clauses.append(
                        ...
                        t.min_squad_size <= (
                            ...
                            WHERE p.tournament_id = s.tournament_id
                                AND p.registration_id = s.id
                                AND p.is_invite = 0
                        )
                        """
                    )
            ...
            async with db.execute(
                f"""SELECT s.id, s.name, s.tag, s.color, s.timestamp, s.is_registered, s.is_approved
                ...
                for row in rows:
                    ...
                    curr_squad = TournamentSquadDetails(
                        registration_id,
                        squad_name,
                        squad_tag,
                        squad_color,
            ...
            if teams_allowed:
                async with db.execute(
                    f"""SELECT tsr.registration_id, tr.id, tr.team_id, tr.name, tr.tag, tr.color, t.name, t.tag, t.color
            ...
            async with db.execute(
                """
                SELECT t.id, t.player_id, t.registration_id, t.is_squad_captain, t.is_representative, t.timestamp,
                    t.is_checked_in, t.mii_name, t.can_host, t.is_invite, t.selected_fc_id, t.is_bagger_clause,
                ...
                player_fc_dict: dict[
                    int, list[FriendCode]
                ] = {}  # create a dictionary of player fcs so we can give all players their FCs
                for row in rows:
                    (
                        reg_id,
                        player_id,
            ...
            fc_query = """
                SELECT id, player_id, type, fc, is_verified, is_primary, description, is_active, creation_date
                FROM friend_codes f
                WHERE f.type = ?
                    AND player_id IN (
                        SELECT t.player_id
            ...
            async with db.execute(fc_query, (fc_type, self.tournament_id)) as cursor:
                ...
                for row in rows:
                    ...
                    if player_id not in player_fc_dict:
                        continue
                    player_fc_dict[player_id].append(
                        FriendCode(
                            fc_id,
                            fc,
                            type,
...
class GetPlayerRegistrationCommand(Command[MyTournamentRegistrationDetails]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                "SELECT game, teams_allowed FROM tournaments WHERE id = ?",
                (self.tournament_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
            ...
            # is_squad_captain: whether the player is either actually captain of the squad, or
            # they have register tournament permissions for at least one roster in the squad.
            # is_invite: just checks if the player is invited, it's put in the main registration class
            # for convenience on the frontend
            async with db.execute(
                f"""SELECT id, name, tag, color, timestamp, is_registered, is_approved,
                                    CASE
                                        WHEN (
                                            s.id IN (
                                                SELECT p.registration_id FROM tournament_players p
                ...
                for row in rows:
                    ...
                    squads[registration_id] = curr_squad
                    details.registrations.append(
                        MyTournamentRegistration(
                            curr_squad,
                            None,
                            bool(is_squad_captain),
                            bool(is_invite),
                        )
                    )
            # get teams connected to squads
            if teams_allowed:
                async with db.execute(
                    f"""SELECT tsr.registration_id, tr.id, tr.team_id, tr.name, tr.tag, t.name, t.tag, t.color
                                    FROM team_squad_registrations tsr
                                    JOIN team_rosters tr ON tsr.roster_id = tr.id
                                    JOIN teams t ON tr.team_id = t.id
                                    WHERE tsr.registration_id IN (
...
class TogglePlayerCheckinCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT checkins_enabled, checkins_open FROM tournaments WHERE id = ?",
                (self.tournament_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
                ...
                if not checkins_enabled:
                    raise Problem(
                        "Checkins are not enabled for this tournament",
                        status=400,
                    )

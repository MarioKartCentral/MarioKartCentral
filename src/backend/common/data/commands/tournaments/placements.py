...
class SetTournamentPlacementsCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        sorted_placements = sorted(
            b,
            key=lambda x: float("inf") if x.placement is None else x.placement,
        )
        ...
        for i, placement in enumerate(sorted_placements):
            if placement.placement_description and len(placement.placement_description) > 32:
                raise Problem(
                    "Placement descriptions must be 32 characters or less",
                    status=400,
                )
            if placement.placement is None:
                if not placement.is_disqualified:
                    raise Problem(
                        "Cannot have null placement value when not disqualified",
                        status=400,
                    )
                if placement.placement_lower_bound:
                    raise Problem(
                        "Placement must be non-null to set lower bound",
                        status=400,
                    )
                if placement.placement_description:
                    raise Problem(
                        "Cannot have description for null placements",
                        status=400,
                    )
            if placement.placement and placement.is_disqualified:
                raise Problem(
                    "Cannot have a placement value while disqualified",
                    status=400,
                )
            if placement.registration_id not in registration_dict.keys():
                raise Problem(
                    f"Registration ID {placement.registration_id} not found in tournament",
                    status=400,
                )
            # if the current placement is not equal to its sorted position
            # in the list or is not tied with the previous placement
            if (
                placement.placement is not None
                and placement.placement != i + 1
                and placement.placement != curr_placement
            ):
                raise Problem(
                    f"""Error processing placements: Placement for ID {placement.registration_id}
                    has value {placement.placement} instead of {i + 1} or {curr_placement}""",
                    status=400,
                )
            if i > 0 and placement.placement == curr_placement and placement.placement_lower_bound != curr_bound:
                raise Problem(
                    f"""All entries with the same placement must have the same lower bound
                    as well (two placements found with bounds {curr_placement}-{curr_bound}
                    and {placement.placement}-{placement.placement_lower_bound})""",
                    status=400,
                )
            # example: we should have exactly 3 placements with placement 3 and lower bound 5,
            # so if the placement changes early throw a error
            if placement.placement != curr_placement and curr_bound is not None and i != curr_bound:
                raise Problem(
                    f"""Expected {curr_bound - curr_placement + 1} placements with value {curr_placement}-{curr_bound},
                    but only found {i - curr_placement + 1}""",
                    status=400,
                )
            ...
            if placement.placement_lower_bound != curr_bound and placement.placement_lower_bound is not None:
                if placement.placement_lower_bound <= i + 1:
                    raise Problem(
                        f"""Error in placement for ID {placement.registration_id}: Placement lower bound
                        should always be greater than placement""",
                        status=400,
                    )
                if placement.placement_lower_bound >= len(sorted_placements):
                    raise Problem(
                        f"""Error in placement for ID {placement.registration_id}: Placement lower bound
                        should not be higher than total num of placements""",
                        status=400,
                    )
        ...
        async with db_wrapper.connect() as db:
            params = [
                (
                    ...
                    p.registration_id,
                    p.placement,
                    p.placement_description,
                    p.placement_lower_bound,
                    p.is_disqualified,
                )
                for p in b
            ]
            await db.execute(
                "DELETE FROM tournament_placements WHERE tournament_id = ?",
                (self.tournament_id,),
            )
            await db.executemany(
                """
                INSERT INTO tournament_placements(
                    tournament_id, registration_id, placement, placement_description,
                    placement_lower_bound, is_disqualified)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                params,
            )
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT min_squad_size, max_squad_size, is_squad FROM tournaments WHERE id = ?",
                (self.tournament_id,),
            ) as cursor:
        ...
        for i, placement in enumerate(sorted_placements):
            ...
            if is_squad:
                if min_squad_size is not None and len(placement.player_ids) < min_squad_size:
                    raise Problem(
                        f"""Error in placement for ID {placement.player_ids}:
                        Row has less players than minimum squad size""",
                        status=400,
                    )
                if max_squad_size is not None and len(placement.player_ids) > max_squad_size:
                    raise Problem(
                        f"""Error in placement for ID {placement.player_ids}:
                        Row has more players than maximum squad size""",
                        status=400,
                    )
            else:
                if len(placement.player_ids) > 1:
                    raise Problem(
                        f"""Error in placement for ID {placement.player_ids}:
                        Row has more than 1 player for a solo tournament""",
                        status=400,
                    )
            if placement.placement and placement.is_disqualified:
                raise Problem(
            ...
            if (
                ...
                raise Problem(
                    f"""Error processing placements: Placement for ID {placement.player_ids}
                    has value {placement.placement} instead of {i + 1} or {curr_placement}""",
                    status=400,
                )
            if i > 0 and placement.placement == curr_placement and placement.placement_lower_bound != curr_bound:
                raise Problem(
                    f"""All entries with the same placement must have the same lower bound as well
                    (two placements found with bounds {curr_placement}-{curr_bound} 
                    and {placement.placement}-{placement.placement_lower_bound})""",
                    status=400,
                )
            # example: we should have exactly 3 placements with placement 3 and lower bound 5,
            ...
            if placement.placement_lower_bound != curr_bound and placement.placement_lower_bound is not None:
                if placement.placement_lower_bound <= i + 1:
                    raise Problem(
                        f"""Error in placement for ID {placement.player_ids}: Placement lower bound
                        should always be greater than placement""",
                        status=400,
                    )
                if placement.placement_lower_bound >= len(sorted_placements):
        ...
        async with db_wrapper.connect() as db:
            ...
            for placement in sorted_placements:
                async with db.execute(
                    """INSERT INTO tournament_registrations(color, timestamp, tournament_id, is_registered, is_approved)
                                    VALUES(?, ?, ?, ?, ?)""",
                    (0, now, self.tournament_id, True, True),
                ) as cursor:
            ...
            player_rows = [
                ...
                for registration_id, placement in squad_dict.items()
                for player_id in placement.player_ids
            ]
            await db.executemany(
                """
            ...
            placement_rows = [
                (
                    ...
                    placement.is_disqualified,
                )
                for registration_id, placement in squad_dict.items()
            ]
            await db.executemany(
                """INSERT INTO tournament_placements(tournament_id, registration_id, placement, placement_description, 
                                 placement_lower_bound, is_disqualified) VALUES(?, ?, ?, ?, ?, ?)""",
                placement_rows,
            )
            await db.commit()
...
class GetTournamentPlacementsCommand(Command[TournamentPlacementList]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                """
                SELECT registration_id, placement, placement_description,
                    placement_lower_bound, is_disqualified
                ...
                unplaced_squads = [
                    TournamentPlacementDetailed(squad.id, None, None, None, False, squad)
                    for squad in self.squads
                    if squad.id not in placed_ids
                ]
...
class GetPlayerTournamentPlacementsCommand(Command[PlayerTournamentResults]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(readonly=True) as db:
            ...
            async with db.execute(
                """
                SELECT t.id as "tournament_id", t.name as "tournament_name", t.game, t.mode,
                    s.id, s.name, t.date_start, t.date_end, tsp.placement,
                    tsp.placement_description, tsp.is_disqualified
                FROM tournament_players as tp 
                INNER JOIN tournaments as t
                ON tp.tournament_id = t.id
                JOIN tournament_registrations s
                    ON s.id = tp.registration_id
                ...
                for row in rows:
                    ...
                    squad_placement = PlayerTournamentPlacement(
                        tournament_id,
                        tournament_name,
                        game,
                        mode,
            ...
            async with db.execute(
                ...
                WHERE tsp.registration_id IN (
                        SELECT registration_id
                        FROM tournament_players
                        WHERE player_id = ?
                    )
                    AND tsp.registration_id NOT IN (
                        SELECT registration_id
            ...
            async with db.execute(
                """
                SELECT DISTINCT t.id, t.name, t.game, t.mode, s.id, s.name, t.date_start,
                    t.date_end, tsp.placement, tsp.placement_description, tsp.is_disqualified
                FROM tournament_players as tp 
                INNER JOIN tournaments as t
                    ON tp.tournament_id = t.id 
                JOIN tournament_registrations s
                ...
                LEFT JOIN team_rosters as tr
                    ON tsr.roster_id = tr.id
                LEFT JOIN teams
                ...
                for row in rows:
                    ...
                    placement_obj = PlayerTournamentPlacement(
                        tournament_id,
                        tournament_name,
                        game,
                        mode,
...
class GetTeamTournamentPlacementsCommand(Command[TeamTournamentResults]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(readonly=True) as db:
            ...
            async with db.execute(
                """
                SELECT DISTINCT s.id, s.name, t.id as "tournament_id", t.name as "tournament_name", t.game, t.mode,
                    t.date_start, t.date_end, tsp.placement, tsp.placement_description, tsp.is_disqualified
                ...
                for row in rows:
                    ...
                    placement_obj = TeamTournamentPlacement(
                        tournament_id,
                        tournament_name,
                        game,
                        mode,

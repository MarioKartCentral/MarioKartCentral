...
from common.data.models import (
    ...
    SeriesBasic,
    SeriesFilter,
    EditSeriesRequestData,
    CreateSeriesRequestData,
    SeriesS3Fields,
    User,
)
        ...
        async with db_wrapper.connect() as db:
            cursor = await db.execute(
                ...
                INSERT INTO tournament_series(
                    name, url, display_order, game, mode, is_historical, is_public,
                    short_description, logo, organizer, location, discord_invite)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    b.series_name,
            ...
            if b.logo_file:
                await db.execute(
                    "UPDATE tournament_series SET logo = ? WHERE id = ?",
                    (logo_path, series_id),
                )
    ...
    async def handle(self, db_wrapper: DBWrapper, s3_wrapper: S3Wrapper):
        ...
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT logo FROM tournament_series WHERE id = ?",
                (self.series_id,),
            ) as cursor:
...
class GetSeriesDataCommand(Command[Series]):
    ...
    async def handle(self, db_wrapper: DBWrapper, s3_wrapper: S3Wrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                """SELECT id, name, url, display_order, game, mode, is_historical, is_public, short_description,
                                    logo, organizer, location, discord_invite FROM tournament_series WHERE id = ?""",
                (self.series_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
...
class GetSeriesListCommand(Command[list[SeriesBasic]]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(readonly=True) as db:
            ...
            series_query = f"""
                SELECT id, name, url, display_order, game, mode, is_historical,
                    is_public, short_description, logo, organizer, location, discord_invite 
...
class CheckSeriesVisibilityCommand(Command[bool]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                "SELECT is_public FROM tournament_series WHERE id = ?",
                (self.series_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if row is None:

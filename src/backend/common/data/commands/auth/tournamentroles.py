    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                "SELECT name, position FROM tournament_roles WHERE id = ?",
                (self.role_id,),
            ) as cursor:
            ...
            async with db.execute(
                ...
                WHERE ur.role_id = ?
                    AND ur.tournament_id = ?
                """,
                (self.role_id, self.tournament_id),
            ) as cursor:
                rows = await cursor.fetchall()
                for row in rows:
                    (
...
class GrantTournamentRoleCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper) -> None:
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                "SELECT user_id FROM user_tournament_roles WHERE user_id = ? AND role_id = ? AND tournament_id = ?",
                (target_user_id, role_id, self.tournament_id),
            ) as cursor:
                row = await cursor.fetchone()
                if row:
                    raise Problem(f"Player already has role {self.role}", status=400)

            # if we have the tournament_permissions.MANAGE_TOURNAMENT_ROLES global permission,
            # bypass series/tournament role checks
            async with db.execute(
                """
                SELECT EXISTS (
            ...
            if not is_mod:
                # if we have the tournament_permissions.MANAGE_TOURNAMENT_ROLES series permission,
                # bypass tournament role checks
                async with db.execute(
                    """
                    SELECT EXISTS (
            ...
            try:
                await db.execute(
                    """
                    INSERT INTO user_tournament_roles(user_id, role_id, tournament_id, expires_on)
                    VALUES (?, ?, ?, ?)
                    """,
                    (
                        target_user_id,
...
class RemoveTournamentRoleCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper) -> None:
        async with db_wrapper.connect() as db:
            ...
            try:
                await db.execute(
                    "DELETE FROM user_tournament_roles WHERE user_id = ? AND role_id = ? AND tournament_id = ?",
                    (target_user_id, role_id, self.tournament_id),
                )
                await db.commit()
            except Exception:

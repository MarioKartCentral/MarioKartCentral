        ...
        async with db_wrapper.connect(db_name="main", attach=["auth"]) as db:
            ...
            if self.mod_user_id != self.user_id:
                ...
                async with db.execute(
                    ...
                    if not row or not row[0]:
                        raise Problem(
                            "Cannot grant API tokens to users higher/equal in the role hierarchy to yourself",
                            status=403,
                        )
            async with db.execute(
                "SELECT name FROM auth.api_tokens WHERE user_id = ? AND name = ?",
                (self.user_id, self.name),
            ) as cursor:
                row = await cursor.fetchone()
                if row:
                    raise Problem(
                        "Token already exists with this name for this user",
                        status=400,
                    )
            api_token = secrets.token_hex(16)
            await db.execute(
                "INSERT INTO auth.api_tokens(token_id, user_id, name) VALUES(:token_id, :user_id, :name)",
                {
                    "token_id": api_token,
...
class DeleteAPITokenCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(db_name="auth") as db:
            ...
            await db.execute(
                "DELETE FROM api_tokens WHERE token_id = ? AND user_id = ?",
                (self.token_id, self.user_id),
            )
            await db.commit()
...
class GetUserFromAPITokenCommand(Command[User | None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(db_name="main", attach=["auth"]) as db:
            async with db.execute(
                "SELECT u.id, u.player_id FROM users u JOIN auth.api_tokens a ON u.id = a.user_id WHERE token_id = ?",
                (self.token_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
...
class GetUserAPITokensCommand(Command[list[APIToken]]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(db_name="auth") as db:
            async with db.execute(
                "SELECT token_id, user_id, name FROM api_tokens WHERE user_id = ?",
                (self.user_id,),
            ) as cursor:

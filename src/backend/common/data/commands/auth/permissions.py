    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            ...
            if self.tournament_id and not series_id:
                async with db.execute(
                    "SELECT series_id FROM tournaments WHERE id = ?",
                    (self.tournament_id,),
                ) as cursor:
            ...
            if series_id:
                ...
                async with db.execute(
                    ...
                    WHERE ur.user_id = ?
                        ...
                        AND p.name = ?
                    """,
                    (self.user_id, series_id, self.permission_name),
                ) as cursor:
                    rows = await cursor.fetchall()
            ...
            if self.tournament_id:
                ...
                async with db.execute(
                    ...
                    WHERE ur.user_id = ?
                        ...
                        AND p.name = ?
                    """,
                    (self.user_id, self.tournament_id, self.permission_name),
                ) as cursor:
                    rows = await cursor.fetchall()
                perm_check = check_perms(rows)
                if perm_check is not None:
            ...
            if self.team_id:
                ...
                async with db.execute(
                    ...
                    WHERE ur.user_id = ?
                        ...
                        AND p.name = ?
                    """,
                    (self.user_id, self.team_id, self.permission_name),
                ) as cursor:
                    rows = await cursor.fetchall()

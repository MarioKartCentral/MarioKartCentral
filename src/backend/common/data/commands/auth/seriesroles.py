    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                "SELECT name, position FROM series_roles WHERE id = ?",
                (self.role_id,),
            ) as cursor:
...
class GrantSeriesRoleCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper) -> None:
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                "SELECT user_id FROM user_series_roles WHERE user_id = ? AND role_id = ? AND series_id = ?",
                (target_user_id, role_id, self.series_id),
            ) as cursor:
                row = await cursor.fetchone()
                if row:
            ...
            async with db.execute(
                ...
                """,
                (self.granter_user_id, series_permissions.MANAGE_SERIES_ROLES),
            ) as cursor:
                row = await cursor.fetchone()
            ...
            try:
                await db.execute(
                    "INSERT INTO user_series_roles(user_id, role_id, series_id, expires_on) VALUES (?, ?, ?, ?)",
                    (target_user_id, role_id, self.series_id, self.expires_on),
                )
                await db.commit()
            except Exception:
    ...
    async def handle(self, db_wrapper: DBWrapper) -> None:
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                ...
                """,
                (self.remover_user_id, series_permissions.MANAGE_SERIES_ROLES),
            ) as cursor:
                row = await cursor.fetchone()
                is_mod = row is not None and bool(row[0])
            ...
            try:
                await db.execute(
                    "DELETE FROM user_series_roles WHERE user_id = ? AND role_id = ? AND series_id = ?",
                    (target_user_id, role_id, self.series_id),
                )
                await db.commit()
            except Exception:

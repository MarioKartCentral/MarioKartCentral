    ...
    async def handle(self, db_wrapper: DBWrapper) -> TeamRoleInfo:
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                "SELECT name, position FROM team_roles WHERE id = ?",
                (self.role_id,),
            ) as cursor:
    ...
    async def handle(self, db_wrapper: DBWrapper) -> None:
        async with db_wrapper.connect() as db:
                    ...
                    FROM (
                        ...
                        WHERE utr.role_id = ?
                            ...
                            AND tm.roster_id IN (
                                ...
                                WHERE tm.leave_date IS NULL
                                    AND tm.player_id = ?
                            )
                        GROUP BY tr.id
                    )""",
                    (role_id, self.team_id, self.target_player_id),
                ) as cursor:
                    row = await cursor.fetchone()
                    if row:
                        count = row[0]
                        ...
                        if count is not None and count >= 4:
                            raise Problem(
                                "Team rosters can only have a maximum of 4 leaders",
                                status=400,
                            )

            async with db.execute(
                "SELECT user_id FROM user_team_roles WHERE user_id = ? AND role_id = ? AND team_id = ?",
                (target_user_id, role_id, self.team_id),
            ) as cursor:
                row = await cursor.fetchone()
                if row:
            ...
            async with db.execute(
                ...
                (target_user_id, permissions.CREATE_TEAM),
            ) as cursor:
                row = await cursor.fetchone()
            ...
            async with db.execute(
                ...
                """,
                (self.granter_user_id, team_permissions.MANAGE_TEAM_ROLES),
            ) as cursor:
                row = await cursor.fetchone()
            ...
            try:
                await db.execute(
                    "INSERT INTO user_team_roles(user_id, role_id, team_id, expires_on) VALUES (?, ?, ?, ?)",
                    (target_user_id, role_id, self.team_id, self.expires_on),
                )
                await db.commit()
            except Exception:
    ...
    async def handle(self, db_wrapper: DBWrapper) -> None:
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                ...
                """,
                (self.remover_user_id, team_permissions.MANAGE_TEAM_ROLES),
            ) as cursor:
                row = await cursor.fetchone()
                is_mod = row is not None and bool(row[0])
            ...
            try:
                await db.execute(
                    "DELETE FROM user_team_roles WHERE user_id = ? AND role_id = ? AND team_id = ?",
                    (target_user_id, role_id, self.team_id),
                )
                await db.commit()
            except Exception:

...
class CreateFakeUserDiscordCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(db_name="main", attach=["discord_tokens"]) as db:
            await db.execute(
                "DELETE FROM user_discords WHERE user_id = :user_id",
            ...
            await db.execute(
                "DELETE FROM discord_tokens.discord_tokens WHERE user_id = :user_id",
                {"user_id": self.user_id},
            )
                ...
                if not row:
                    ...
                    async with db.execute(account_check, {"user_id": self.user_id}) as check_cursor:
                        if await check_cursor.fetchone():
                            raise Problem(
                                "Discord tokens not found, please relink your Discord account",
                                status=400,
                            )
                        else:
                            raise Problem(
...
class DeleteUserDiscordDataCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper, discord_api: DiscordApi):
        ...
        async with db_wrapper.connect(db_name="main", attach=["discord_tokens"]) as db:
            ...
            await db.execute(
                "UPDATE user_settings SET avatar = NULL WHERE user_id = :user_id",
                {"user_id": self.user_id},
            )
            await db.execute(
                "DELETE FROM discord_tokens.discord_tokens WHERE user_id = :user_id",
    ...
    async def handle(
        ...
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                "SELECT discord_id, avatar FROM user_discords WHERE user_id = ?",
                (self.user_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
...
class RemoveDiscordAvatarCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper, s3_wrapper: S3Wrapper):
        async with db_wrapper.connect() as db:
            ...
            await db.execute(
                "UPDATE user_settings SET avatar = NULL WHERE user_id = ?",
                (user_id,),
            )
            await db.execute(
                "UPDATE user_discords SET avatar = NULL WHERE user_id = ?",
                (user_id,),
            )
            await db.commit()

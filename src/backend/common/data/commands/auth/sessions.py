        ...
        async with db_wrapper.connect(db_name="user_activity") as db:
            ...
            login_command = """
                INSERT INTO user_logins(
                    user_id, ip_address_id, session_id, persistent_session_id,
                    fingerprint, had_persistent_session, date)
                VALUES(
                    :user_id, :ip_address_id, :session_id,
                    :persistent_session_id, :fingerprint, :had_persistent_session, :date)
            """
...
class DeleteSessionCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(db_name="user_activity") as db:
            await db.execute(
                "UPDATE user_logins SET logout_date = :now WHERE session_id = :session_id",
                {"now": now, "session_id": self.session_id},
            )
            await db.commit()

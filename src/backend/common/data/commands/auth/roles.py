    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            ...
            async with db.execute(
                "SELECT id, name, position FROM roles WHERE name != ?",
                (user_roles.BANNED,),
            ) as cursor:
...
class GetRoleInfoCommand(Command[RoleInfo]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                "SELECT name, position FROM roles WHERE id = ?",
                (self.role_id,),
            ) as cursor:
...
@dataclass
class GetUserRolePermissionsCommand(
    Command[
        tuple[
            list[UserRole],
            list[TeamRole],
        ...
        async with db_wrapper.connect() as db:
            ...
            try:
                await db.execute(
                    ...
                    (target_user_id, role_id, self.expires_on),
                )
                await db.commit()
            except Exception:
        ...
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                ...
                SELECT EXISTS(
                    ...
                    JOIN permissions p ON rp.permission_id = p.id
                    JOIN roles r2
                    WHERE ur.user_id = :granter_user_id
                        AND r2.name = :role_name
                        AND r1.position < r2.position
                        AND rp.is_denied = 0
                        AND (:is_ban = 1 OR p.name = :permission_name)
                )
                """,
                {
                    "granter_user_id": self.granter_user_id,
                    "role_name": self.role,
                    "permission_name": permissions.MANAGE_USER_ROLES,
                    "is_ban": self.is_ban,
                },
            ) as cursor:
                row = await cursor.fetchone()
            ...
            try:
                await db.execute(
                    "UPDATE user_roles SET expires_on = ? WHERE user_id = ? AND role_id = ?",
                    (self.expires_on, target_user_id, role_id),
                )
                await db.commit()
            except Exception:
                raise Problem("Unexpected error")
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(query, (timestamp,)) as cursor:
                ...
                if rows:
                    await db.execute(
                        "DELETE FROM user_roles WHERE expires_on < ?",
                        (timestamp,),
                    )
            ...
            async with db.execute(query, (timestamp,)) as cursor:
                ...
                if rows:
                    await db.execute(
                        "DELETE FROM user_team_roles WHERE expires_on < ?",
                        (timestamp,),
                    )
            ...
            async with db.execute(query, (timestamp,)) as cursor:
                ...
                if rows:
                    await db.execute(
                        "DELETE FROM user_series_roles WHERE expires_on < ?",
                        (timestamp,),
                    )
            ...
            async with db.execute(query, (timestamp,)) as cursor:
                ...
                if rows:
                    await db.execute(
                        "DELETE FROM user_tournament_roles WHERE expires_on < ?",
                        (timestamp,),
                    )
            ...
            if notif_rows:
                await db.executemany(
                    """
                    INSERT INTO notifications(user_id, type, content_id, content_args, link, created_date)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    notif_rows,
                )

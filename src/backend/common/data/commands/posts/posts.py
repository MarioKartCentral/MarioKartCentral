    ...
    async def handle(self, db_wrapper: DBWrapper, s3_wrapper: S3Wrapper) -> int:
        async with db_wrapper.connect() as db:
            ...
            row = await db.execute_insert(
                """INSERT INTO posts(title, is_public, is_global, creation_date, created_by)
                VALUES(?, ?, ?, ?, ?)""",
                (
                    self.title,
                    self.is_public,
            ...
            if self.series_id:
                await db.execute(
                    "INSERT INTO series_posts(series_id, post_id) VALUES(?, ?)",
                    (self.series_id, post_id),
                )
            if self.tournament_id:
                await db.execute(
                    "INSERT INTO tournament_posts(tournament_id, post_id) VALUES(?, ?)",
                    (self.tournament_id, post_id),
                )
            await db.commit()
    ...
    async def handle(self, db_wrapper: DBWrapper, s3_wrapper: S3Wrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                """UPDATE posts SET title = :title, is_public = :is_public, is_global = :is_global
                                WHERE id = :id
...
class ListPostsCommand(Command[PostList]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        filter = self.filter
        query = """
        SELECT DISTINCT p.id, p.title, p.is_public, p.is_global,
            p.creation_date, pl.id, pl.name, pl.country_code, pl.is_banned
        FROM posts p
...
class GetPostCommand(Command[Post]):
    ...
    async def handle(self, db_wrapper: DBWrapper, s3_wrapper: S3Wrapper):
        query = """
        SELECT p.id, p.title, p.is_public, p.is_global, p.creation_date, pl.id, pl.name, pl.country_code, pl.is_banned
        FROM posts p
        LEFT JOIN players pl ON p.created_by = pl.id
        WHERE p.id = :id

        ...
        async with db_wrapper.connect(db_name="main", attach=["auth"]) as db:
            ...
            for type in fc_limits.keys():
                ...
                if len(game_fcs) > fc_limits[type]:
                    raise Problem(
                        f"Too many friend codes were provided for the FC type {type} (limit {fc_limits[type]})",
                        status=400,
                    )
            ...
            for friend_code in self.friend_codes:
                ...
                if friend_code.type != "nnid" and not match:
                    raise Problem(
                        f"FC {friend_code.fc} of type {friend_code.type} is in incorrect format",
                        status=400,
                    )
                ...
                async with db.execute(
                    "SELECT id FROM friend_codes WHERE fc = ? AND type = ? AND is_active = ?",
                    (friend_code.fc, friend_code.type, True),
                ) as cursor:
                    row = await cursor.fetchone()
                    if row:
                        raise Problem(
                            f"Another player is currently using this {friend_code.type} friend code",
                            status=400,
                        )
            ...
            friend_code_tuples = [
                (
                    ...
                    friend_code.fc,
                    False,
                    friend_code.is_primary,
                    True,
                    friend_code.description,
                    now,
                )
                for friend_code in self.friend_codes
            ]
    ...
    async def handle(self, db_wrapper: DBWrapper) -> PlayerUpdate | None:
        ...
        async with db_wrapper.connect() as db:
            ...
            if curr_name != data.name:
                now = int(datetime.now(timezone.utc).timestamp())
                await db.execute(
                    """INSERT INTO player_name_edits(player_id, old_name, new_name, date, approval_status, handled_by)
                                    VALUES(?, ?, ?, ?, ?, ?)""",
                    (
                        data.player_id,
                        curr_name,
...
class GetPlayerDetailedCommand(Command[PlayerDetailed | None]):
    ...
    async def handle(self, db_wrapper: DBWrapper) -> PlayerDetailed | None:
        async with db_wrapper.connect(readonly=True) as db:
            ...
            fc_query = """
            SELECT id, type, fc, is_verified, is_primary, description, is_active, creation_date
            FROM friend_codes
            WHERE player_id = ?
            """
            ...
            friend_codes = [
                ...
                for id, type, fc, is_verified, is_primary, description, is_active, creation_date in friend_code_rows
            ]
            ...
            async with db.execute(
                """
                SELECT m.roster_id, m.join_date, t.id, t.name, t.tag, t.color,
                    r.name, r.tag, r.game, r.mode, r.color, m.is_bagger_clause
                FROM team_members m
            ...
            async with db.execute(
                """
                SELECT id, old_name, new_name, date, approval_status
                FROM player_name_edits
                WHERE player_id = ?
            ...
            if self.include_notes:
                ...
                async with db_wrapper.connect(db_name="main", attach=["player_notes"]) as db_with_notes:
                    async with db_with_notes.execute(
                        "SELECT notes, edited_by, date FROM player_notes.player_notes WHERE player_id = ?",
                        (self.id,),
                    ) as cursor:
                        row = await cursor.fetchone()
                        if row:
                            ...
                            query = """
                            SELECT p.id, p.name, p.country_code, p.is_hidden,
                                p.is_shadow, p.is_banned, p.join_date FROM players p 
    ...
    async def handle(self, db_wrapper: DBWrapper) -> PlayerList:
        ...
        async with db_wrapper.connect(readonly=True) as db:
            ...
            player_from_where_clause = (
                f"{player_where_clause} ORDER BY {order_by} COLLATE NOCASE {desc} LIMIT ? OFFSET ?"
            )
            ...
            friend_codes_query = f"""
            SELECT f.id, f.fc, f.type, f.player_id, f.is_verified,
                f.is_primary, f.description, f.is_active, f.creation_date
            FROM friend_codes f
            WHERE f.player_id IN (
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            async with db.execute("SELECT id FROM players WHERE id = ?", (self.from_player_id,)) as cursor:
                ...
                if not row:
                    raise Problem(
                        f"Player with ID {self.from_player_id} not found",
                        status=404,
                    )
            async with db.execute("SELECT id FROM players WHERE id = ?", (self.to_player_id,)) as cursor:
                ...
                if not row:
                    raise Problem(
                        f"Player with ID {self.to_player_id} not found",
                        status=404,
                    )
            ...
            await db.execute(
                ...
                (self.to_player_id, self.from_player_id),
            )
            await db.execute(
            ...
            await db.execute(
                "UPDATE player_bans SET player_id = ? WHERE player_id = ?",
                (self.to_player_id, self.from_player_id),
            )
            await db.execute(
                "UPDATE player_bans_historical SET player_id = ? WHERE player_id = ?",
                (self.to_player_id, self.from_player_id),
            )
            await db.execute(
                "UPDATE player_name_edits SET player_id = ? WHERE player_id = ?",
                (self.to_player_id, self.from_player_id),
            )
            await db.execute(
                "UPDATE users SET player_id = ? WHERE player_id = ?",
                (None, self.from_player_id),
            )
...
class GetPlayerTransferHistoryCommand(Command[PlayerTransferHistory]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect(readonly=True) as db:
            async with db.execute(
                """
                SELECT t.id, t.name as "team_name", tr.game, tr.mode, tm.id, tm.join_date,
                    tm.leave_date, tm.is_bagger_clause, tm.is_hidden, tr.name as "roster_name"
...
class ToggleTransferHistoryItemVisibilityCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            await db.execute(
                "UPDATE team_members SET is_hidden = NOT is_hidden WHERE id = ? AND player_id = ?",
                (self.item_id, self.player_id),
            )
            await db.commit()

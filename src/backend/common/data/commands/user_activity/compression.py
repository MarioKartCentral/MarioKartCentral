...
class CompressUserActivityTimeRangesCommand(Command[None]):
    ...
    def _get_compression_boundaries(
        self,
    ) -> list[tuple[int, int, GranularityLevel]]:
        """
        ...
        boundaries = [
            ...
            (
                ts_ago(timedelta(days=30)),
                ts_ago(timedelta(days=2)),
                GranularityLevel.ONE_HOUR,
            ),
            (
                ts_ago(timedelta(days=2)),
                ts_ago(timedelta(hours=6)),
                GranularityLevel.THIRTY_MINUTES,
            ),
            (
                ts_ago(timedelta(hours=6)),
                ts_ago(timedelta(hours=1)),
                GranularityLevel.TEN_MINUTES,
            ),
            (
                ts_ago(timedelta(hours=1)),
                ts_ago(timedelta(minutes=10)),
                GranularityLevel.ONE_MINUTE,
            ),

    ...
    async def handle(self, db_wrapper: DBWrapper, s3_wrapper: S3Wrapper):
        async with db_wrapper.connect() as db:
            ...
            if len(self.description) > 500:
                raise Problem(
                    "Team description must be 500 characters or less",
                    status=400,
                )
            # we don't want users to be able to create teams that share the same name/tag as another team for this game,
            # but it should be possible if moderators wish
            if not self.is_privileged:
                async with db.execute(
                    ...
                    WHERE (
                        (r.name IS NOT NULL AND r.name = ?)
                        OR (r.name IS NULL AND t.name = ?) 
                        OR (r.tag IS NOT NULL AND r.tag = ?)
                        OR (r.tag IS NULL AND t.tag = ?)
                    )
                        AND r.game = ?
                        AND r.mode = ?
                        AND r.is_active = 1
                        AND t.is_historical = 0
                        AND t.approval_status != 'denied'
                    """,
                    (name, name, tag, tag, self.game, self.mode),
                ) as cursor:
                    row = await cursor.fetchone()
                    assert row is not None
                    if row[0] > 0:
                        raise Problem(
                            "An existing team for this game/mode already has this name or tag",
                            status=400,
                        )
            if not self.is_privileged and self.user_id is not None:
                async with db.execute(
                    """SELECT t.id FROM teams t
                                        ...
                                        WHERE t.approval_status = 'pending' AND ut.user_id = ?""",
                    (self.user_id,),
                ) as cursor:
                    row = await cursor.fetchone()
                    if row:
                        raise Problem(
                            """You already have a pending team in the approval queue; you must wait for a staff member
                            to approve/deny this team before creating another team""",
                            status=400,
                        )
            ...
            async with db.execute(
                """
                INSERT INTO teams (
                    name, tag, description, creation_date, language,
                    color, logo, approval_status, is_historical)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    name,
            ...
            await db.execute(
                ...
                INSERT INTO team_rosters(
                    team_id, game, mode, name, tag, color, creation_date,
                    is_recruiting, is_active, approval_status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    team_id,
            ...
            if self.user_id is not None:
                await db.execute(
                    "INSERT INTO user_team_roles(user_id, role_id, team_id) VALUES (?, 0, ?)",
...
class GetTeamInfoCommand(Command[Team]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                """
                SELECT name, tag, description, creation_date, language,
                    color, logo, approval_status, is_historical
                FROM teams
                WHERE id = ?
                """,
            ...
            async with db.execute(
                """
                SELECT p.id, p.name, p.country_code, p.is_hidden, p.is_shadow, p.is_banned, p.join_date, tr.name
                FROM players p
                JOIN users u ON u.player_id = p.id
                JOIN user_team_roles ur ON ur.user_id = u.id
                JOIN team_roles tr ON tr.id = ur.role_id
                WHERE ur.team_id = ?
                    AND (tr.name = ? OR tr.name = ?)
                """,
                (self.team_id, team_roles.MANAGER, team_roles.LEADER),
            ) as cursor:
            ...
            async with db.execute(
                """
                SELECT id, game, mode, name, tag, color, creation_date, is_recruiting, is_active, approval_status
                FROM team_rosters
                WHERE team_id = ?
                """,
                (self.team_id,),
            ...
            async with db.execute(
                f"""SELECT m.player_id, m.roster_id, m.join_date, m.is_bagger_clause
                                    FROM team_members m
            ...
            if len(team_members) > 0 or len(team_invites) > 0:
                ...
                member_id_query = ",".join(
                    set([str(m.player_id) for m in team_members] + [str(m.player_id) for m in team_invites])
                )
                async with db.execute(f"""SELECT p.id, p.name, p.country_code, p.is_banned, 
                    ...
                    for row in rows:
                        ...
                        player_dict[player_id] = PartialPlayer(
                            player_id,
                            player_name,
                            country,
                            bool(is_banned),
                ...
                async with db.execute(
                    f"""
                    SELECT id, player_id, type, fc, is_verified, is_primary, is_active, creation_date
                    FROM friend_codes
                    WHERE player_id IN ({member_id_query})
                        AND type IN ({fc_type_query})
                    """
                ) as cursor:
                    rows = await cursor.fetchall()
                    for row in rows:
            ...
            for member in team_members:
                ...
                curr_player = RosterPlayerInfo(
                    ...
                    is_manager,
                    is_leader,
                    member.is_bagger_clause,
                    [fc for fc in p.friend_codes if fc.type == game_fc_map[curr_roster.game]],
                )  # only add FCs that are for the same game as current roster
            ...
            for invite in team_invites:
                ...
                curr_player = RosterInvitedPlayer(
                    p.player_id,
                    p.name,
                    p.country_code,
                    p.is_banned,
...
class EditTeamCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper, s3_wrapper: S3Wrapper):
        ...
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT name, tag, logo FROM teams WHERE id = ?",
                (self.team_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
            ...
            if team_name != self.name or team_tag != self.tag:
                ...
                await db.execute(
                    """
                    INSERT INTO team_edits(
                        team_id, old_name, new_name, old_tag, new_tag, date, approval_status, handled_by)
                    VALUES(?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        self.team_id,
            ...
            # if team is approved, approve all rosters that are pending;
            # if team is not approved, change all approved rosters to pending
...
class ApproveDenyTeamCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "UPDATE teams SET approval_status = ? WHERE id = ?",
                (self.approval_status, self.team_id),
            ) as cursor:
            ...
            if self.approval_status == "approved":
                await db.execute(
                    "UPDATE team_rosters SET approval_status = ? WHERE team_id = ? AND approval_status = 'pending'",
                    (self.approval_status, self.team_id),
                )
            await db.commit()
...
class RequestEditTeamCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT name, tag, approval_status FROM teams WHERE id = ?",
                (self.team_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
                ...
                if self.name == name and self.tag == tag:
                    raise Problem(
                        "At least one of the team name or tag must be different from their current values",
                        status=400,
                    )
                if approval_status != "approved":
                    raise Problem(
                        "Cannot request to edit team if it is not approved",
                        status=400,
                    )
            # check if this team has made a request in the last 90 days
            async with db.execute(
                "SELECT date FROM team_edits WHERE team_id = ? AND date > ? AND approval_status != 'denied' LIMIT 1",
                (
                    self.team_id,
                    (datetime.now(timezone.utc) - timedelta(minutes=90)).timestamp(),
                ),
            ) as cursor:
                row = await cursor.fetchone()
                if row:
            ...
            await db.execute(
                """
                INSERT INTO team_edits(team_id, old_name, new_name, old_tag, new_tag, date, approval_status)
                VALUES(?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    self.team_id,
...
class ViewTeamEditHistoryCommand(Command[list[TeamEdit]]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            async with db.execute(
                """
                SELECT r.id, r.team_id, r.old_name, r.new_name, r.old_tag, r.new_tag, r.date,
                    r.approval_status, r.handled_by, p.name, p.country_code, p.is_banned, t.color
                FROM team_edits r
                JOIN teams t ON r.team_id = t.id
...
class ApproveTeamEditCommand(Command[TeamEditUpdate]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT team_id, new_name, new_tag FROM team_edits WHERE id = ?",
                (self.request_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if row is None:
            ...
            await db.execute(
                "UPDATE teams SET name = ?, tag = ? WHERE id = ?",
                (name, tag, team_id),
            )
            await db.execute(
                "UPDATE team_edits SET approval_status = 'approved', handled_by = ? WHERE id = ?",
...
class DenyTeamEditCommand(Command[TeamEditUpdate]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "UPDATE team_edits SET approval_status = 'denied', handled_by = ? WHERE id = ?",
                (self.mod_player_id, self.request_id),
            ) as cursor:
...
class ListTeamsCommand(Command[TeamList]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            ...
            def append_team_roster_like_filter(
                filter_query: dict[str, list[str]],
                filter_value: Any,
                column_name: str,
            ) -> None:
            ...
            """
            where_clause = (
                "" if not filter_query["where_clauses"] else f"WHERE {' AND '.join(filter_query['where_clauses'])}"
            )
            having_clause = ""
            ...
            async with db.execute(
                team_query,
                (*filter_query["variable_parameters"], limit, offset),
            ) as cursor:
                rows = await cursor.fetchall()
                for row in rows:
                    (
            ...
            rosters_query = f"""
                SELECT r.id, r.team_id, r.game, r.mode, r.name, r.tag,
                    r.color, r.creation_date, r.is_recruiting, r.is_active, r.approval_status
                FROM team_rosters r
            ...
            async with db.execute(rosters_query) as cursor:
                ...
                for row in rows:
                    ...
                    roster = TeamRoster(
                        ...
                        game,
                        mode,
                        name if name else team.name,
                        tag if tag else team.tag,
                        creation_date,
                        bool(is_recruiting),
                        bool(is_active),
                        approval_status,
                        roster_color if roster_color else team.color,
                        [],
                        [],
                    )
...
class MergeTeamsCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT name, tag, color FROM teams WHERE id = ?",
                (self.from_team_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if row is None:
            ...
            await db.execute(
                "UPDATE team_rosters SET name = ? WHERE team_id = ? AND name IS NULL",
                (old_name, self.from_team_id),
            )
            await db.execute(
                "UPDATE team_rosters SET tag = ? WHERE team_id = ? AND tag IS NULL",
                (old_tag, self.from_team_id),
            )
            await db.execute(
                "UPDATE team_rosters SET color = ? WHERE team_id = ? AND color IS NULL",
                (old_color, self.from_team_id),
            )

            await db.execute(
                "UPDATE team_rosters SET team_id = ? WHERE team_id = ?",
                (self.to_team_id, self.from_team_id),
            )
            await db.execute(
                "DELETE FROM user_team_roles WHERE team_id = ?",
                (self.from_team_id,),
            )
            await db.execute(
                "DELETE FROM team_edits WHERE team_id = ?",
                (self.from_team_id,),
            )

    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT team_id, game, approval_status FROM team_rosters WHERE id = ?",
                (self.roster_id,),
            ) as cursor:
                ...
                if approval_status != "approved":
                    raise Problem(
                        "Cannot invite players to roster if it is not approved",
                        status=400,
                    )
                ...
                if self.is_bagger_clause and game != "mkw":
                    raise Problem(
                        "Cannot invite players as baggers for games other than MKW",
                        status=400,
                    )
            async with db.execute(
            ...
            async with db.execute(
                "SELECT id, fc FROM friend_codes WHERE type = ? AND player_id = ? AND is_active = ?",
            ...
            async with db.execute(
                ...
                """,
                (self.player_id, self.roster_id, None, self.is_bagger_clause),
            ) as cursor:
                row = await cursor.fetchone()
                if row:
            ...
            if self.is_bagger_clause:
                ...
                if bagger_count >= 2:
                    raise Problem(
                        "Rosters may only have 2 bag-claused players at once",
                        status=400,
                    )
            creation_date = int(datetime.now(timezone.utc).timestamp())
            async with db.execute(
                """
                INSERT INTO team_transfers(player_id, roster_id, date, is_bagger_clause, is_accepted, approval_status)
                VALUES (?, ?, ?, ?, ?, ?)
                RETURNING date
                """,
                (
...
class DeleteInviteCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT team_id FROM team_rosters WHERE id = ?",
                (self.roster_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
                ...
                if int(roster_team_id) != self.team_id:
                    raise Problem("Roster is not part of specified team", status=400)
            async with db.execute(
                "DELETE FROM team_transfers WHERE player_id = ? AND roster_id = ?",
                (self.player_id, self.roster_id),
            ) as cursor:
...
class AcceptInviteCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                """SELECT r.game, i.player_id, i.is_bagger_clause
            ...
            if self.roster_leave_id:
                async with db.execute(
                    ...
                    if row is None:
                        raise Problem(
                            "Player is not registered for the roster they are leaving",
                            status=400,
                        )
            # make sure we have at least one FC for the game of the roster that we are accepting an invite for
            fc_type = game_fc_map[game]
            async with db.execute(
                "SELECT count(id) FROM friend_codes WHERE player_id = ? AND type = ?",
                (self.player_id, fc_type),
            ) as cursor:
                row = await cursor.fetchone()
                assert row is not None
                fc_count = row[0]
                if fc_count == 0:
                    raise Problem(
                        "Player does not have any friend codes for this roster's game",
                        status=400,
                    )
            now = int(datetime.now(timezone.utc).timestamp())
            # we do not move the player to the team's roster just yet,
            # just mark it as accepted, a moderator must approve the transfer
            await db.execute(
                "UPDATE team_transfers SET roster_leave_id = ?, is_accepted = ?, date = ? WHERE id = ?",
                (self.roster_leave_id, True, now, self.invite_id),
            )
            await db.commit()
...
class DeclineInviteCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            # check if invite exists and to make sure we're the same player as the invite
            async with db.execute(
                "SELECT player_id FROM team_transfers WHERE id = ?",
                (self.invite_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if row is None:

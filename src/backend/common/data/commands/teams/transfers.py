    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                """
                SELECT tt.player_id, tt.roster_id, tt.roster_leave_id, tt.is_accepted,
                    tt.is_bagger_clause, tt.approval_status, r1.team_id, r2.team_id, u.id
                FROM team_transfers tt
                JOIN team_rosters r1 ON tt.roster_id = r1.id
                LEFT JOIN team_rosters r2 ON tt.roster_leave_id = r2.id
                LEFT JOIN users u ON tt.player_id = u.player_id
                WHERE tt.id = ?
                """,
                (self.invite_id,),
            ) as cursor:
            ...
            if not is_accepted:
                raise Problem(
                    "Invite has not been accepted by the player yet",
                    status=400,
                )
            team_member_id = None
            if roster_leave_id:
                ...
                async with db.execute(
                    ...
                    WHERE player_id = ?
                        AND roster_id = ?
                        AND leave_date IS ?
                        AND is_bagger_clause = ?
                    """,
                    (player_id, roster_leave_id, None, is_bagger_clause),
                ) as cursor:
                    row = await cursor.fetchone()
                    # if the player has already left the team, we should still accept the transfer,
                    # but change it to be from no team
                    if row is None:
            ...
            await db.execute(
                "UPDATE team_transfers SET roster_leave_id = ?, approval_status = ? WHERE id = ?",
                (
                    roster_leave_id,
                    "approved",
                    self.invite_id,
            ...
            await db.execute(
                "INSERT INTO team_members(roster_id, player_id, join_date, is_bagger_clause) VALUES (?, ?, ?, ?)",
                (roster_id, player_id, curr_time, is_bagger_clause),
            )
            ...
            async with db.execute(
                ...
                WHERE player_id = ?
                    ...
                    AND is_bagger_clause = ?
                """,
                (curr_time, player_id, roster_id, None, not is_bagger_clause),
            ) as cursor:
                ...
                if rowcount == 1:
                    await db.execute(
                        ...
                        INSERT INTO team_transfers(
                            player_id, roster_id, date, roster_leave_id, is_accepted, approval_status, is_bagger_clause)
                        VALUES(?, ?, ?, ?, ?, ?, ?)
                        """,
                        (
                            player_id,
                            ...
                            True,
                            "approved",
                            not is_bagger_clause,
                        ),
                    )
            # if we are registered for a tournament squad for the team we are transferring to as the
            # opposite bagger type, we should flip the player's tournament registration to the new bagger type
            async with db.execute(
                """
                ...
                WHERE p.player_id = ?
                    ...
                    AND t.team_members_only = 1
                    AND p.is_bagger_clause = ?
                """,
                (player_id, roster_id, not is_bagger_clause),
            ) as cursor:
                rows = await cursor.fetchall()
                bagger_registration_ids: list[tuple[bool, int]] = [
                    (bool(is_bagger_clause), int(row[0])) for row in rows
                ]
            if len(bagger_registration_ids):
                await db.executemany(
                    "UPDATE tournament_players SET is_bagger_clause = ? WHERE id = ?",
                    bagger_registration_ids,
                )
            if team_member_id:
                await db.execute(
                    "UPDATE team_members SET leave_date = ? WHERE id = ?",
                    (curr_time, team_member_id),
                )
                ...
                async with db.execute(
                    ...
                    WHERE p.player_id = ?
                        ...
                        AND p.is_bagger_clause = ?
                    """,
                    (player_id, roster_leave_id, is_bagger_clause),
                ) as cursor:
                    rows = await cursor.fetchall()
                ...
                async with db.execute(
                    ...
                    WHERE p.player_id = ?
                        ...
                        AND p.is_bagger_clause = ?
                    """,
                    (player_id, roster_id, is_bagger_clause),
                ) as cursor:
                    rows = await cursor.fetchall()
                    for row in rows:
                ...
                await db.execute(
                    ...
                    WHERE player_id = ?
                    AND registration_id IN ({",".join(map(str, registration_ids))})
                    """,
                    (player_id,),
                )

                # get all team tournament rosters the player is in where registrations have closed
                # and they should be marked as ineligible
                async with db.execute(
                    """
                    SELECT p.registration_id FROM tournament_players p
                    JOIN team_squad_registrations s ON p.registration_id = s.registration_id
                    ...
                    WHERE p.player_id = ?
                        AND s.roster_id = ?
                        AND t.registrations_open = 0
                        AND t.team_members_only = 1
                        AND t.sync_team_rosters = 1
                        AND t.date_end > ?
                        AND p.is_bagger_clause = ?
                    """,
                    (player_id, roster_leave_id, curr_time, is_bagger_clause),
                ) as cursor:
                    rows = await cursor.fetchall()
                    registration_ids: list[int] = [row[0] for row in rows]
                # if any of these squads the player was in previously are also linked to the new roster,
                ...
                async with db.execute(
                    ...
                    WHERE p.player_id = ?
                        ...
                        AND p.is_bagger_clause = ?
                    """,
                    (player_id, roster_id, curr_time, is_bagger_clause),
                ) as cursor:
                    rows = await cursor.fetchall()
                    for row in rows:
                        if row[0] in registration_ids:
            ...
            # next we want to automatically add the transferred player to any team
            # tournaments where that team is registered and registrations are open
            async with db.execute(
                """
                SELECT s.registration_id, t.id
                FROM team_squad_registrations s
                JOIN tournaments t ON s.tournament_id = t.id
                WHERE t.teams_allowed = ?
                    AND t.registrations_open = ?
                    AND s.roster_id = ?
                    AND NOT EXISTS (
                        SELECT p.id FROM tournament_players p
                        WHERE p.player_id = ?
                            AND p.registration_id = s.registration_id
                    )
                """,
                (True, True, roster_id, player_id),
            ) as cursor:
                rows = await cursor.fetchall()
                squads: dict[int, int] = {}
                for row in rows:
            ...
            async with db.execute(
                f"""
                SELECT registration_id, tournament_id
                FROM tournament_players
                WHERE player_id = ?
                    AND is_bagger_clause = ?
                    AND tournament_id IN ({tournament_query})
                """,
                (player_id, is_bagger_clause),
            ) as cursor:
                rows = await cursor.fetchall()
                for row in rows:
                    squad, tournament = row
            ...
            insert_rows = [
                ...
                for tournament_id, registration_id in squads.items()
            ]
            await db.executemany(
                """
                INSERT INTO tournament_players(
                    player_id, tournament_id, registration_id, is_squad_captain, timestamp,
                    is_checked_in, mii_name, can_host, is_invite, selected_fc_id, 
            ...
            # remove user's team roles if they are no longer in a roster for the old team
            if user_id is not None and leave_team_id and leave_team_id != join_team_id:
                ...
                async with db.execute(
                    ...
                    """,
                    (player_id, roster_leave_id, leave_team_id),
                ) as cursor:
                    row = await cursor.fetchone()
...
class DenyTransferCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                "SELECT player_id, roster_id, roster_leave_id, is_accepted FROM team_transfers WHERE id = ?",
                (self.invite_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if row is None:
                    raise Problem("Invite not found", status=404)
            # we would want to send the invite back to the player if for example
            # they didn't specify a team they're leaving, otherwise just set it to denied
            if self.send_back:
                await db.execute(
                    "UPDATE team_transfers SET is_accepted = ? WHERE id = ?",
                    (False, self.invite_id),
                )
            else:
                await db.execute(
                    "UPDATE team_transfers SET approval_status = 'denied' WHERE id = ?",
                    (self.invite_id,),
                )
            await db.commit()
...
class ViewTransfersCommand(Command[TransferList]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        ...
        async with db_wrapper.connect() as db:
            ...
            async with db.execute(
                f"""SELECT i.id, i.date, i.approval_status, i.is_bagger_clause,
                                    ...
                                    WHERE i.is_accepted = 1
                                        AND i.approval_status = ? {where_clause_str}
                                    ORDER BY i.date DESC LIMIT ? OFFSET ?""",
                (self.approval_status, *variable_parameters, limit, offset),
            ) as cursor:
                rows = await cursor.fetchall()
                for row in rows:
                    (
                    ...
                    if leave_roster_id:
                        leave_roster = RosterBasic(
                            leave_team_id,
                            leave_team_name,
                            leave_team_tag,
                            leave_roster_color if leave_roster_color else leave_team_color,
                            leave_roster_id,
                            leave_roster_name,
                            leave_roster_tag,
                        )
                    ...
                    if join_roster_id:
                        join_roster = RosterBasic(
                            join_team_id,
                            join_team_name,
                            join_team_tag,
                            join_roster_color if join_roster_color else join_team_color,
                            join_roster_id,
                            join_roster_name,
                            join_roster_tag,
                        )
            ...
            async with db.execute(
                ...
                WHERE i.is_accepted = 1
                    ...
                    {where_clause_str}
                """,
                (self.approval_status, *variable_parameters),
            ) as cursor:
                row = await cursor.fetchone()
                assert row is not None
...
class ForceTransferPlayerCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                """SELECT p.id, u.id FROM players p
                ...
                _, user_id = row
            async with db.execute(
                "SELECT game, team_id FROM team_rosters WHERE id = ? AND team_id = ?",
                (self.roster_id, self.team_id),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
                    raise Problem("Roster not found", status=404)
                game, join_team_id = row
                if self.is_bagger_clause and game != "mkw":
                    raise Problem(
                        "Cannot make players baggers for games other than MKW",
                        status=400,
                    )
            fc_type = game_fc_map[game]
            async with db.execute(
                "SELECT id FROM friend_codes WHERE type = ? AND player_id = ? AND is_active = ?",
                (fc_type, self.player_id, True),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
            ...
            if self.roster_leave_id:
                # check this before we move the player to the new team
                async with db.execute(
                    """SELECT m.id, r.team_id FROM team_members m
                                      JOIN team_rosters r ON m.roster_id = r.id
                                      WHERE m.player_id = ? AND m.roster_id = ? AND m.leave_date IS ?""",
                    (self.player_id, self.roster_leave_id, None),
                ) as cursor:
                    row = await cursor.fetchone()
                    if row is None:
                        raise Problem(
                            "Player is not currently on this roster",
                            status=400,
                        )
                    team_member_id, leave_team_id = row
            else:
                team_member_id = None
            async with db.execute(
                "SELECT id FROM team_members WHERE player_id = ? AND roster_id = ? AND leave_date IS ?",
                (self.player_id, self.roster_id, None),
            ) as cursor:
                row = await cursor.fetchone()
                if row:
            ...
            if team_member_id:
                ...
                async with db.execute(
                    ...
                    WHERE p.player_id = ?
                        ...
                        AND p.is_bagger_clause = ?
                    """,
                    (self.player_id, self.roster_id, self.is_bagger_clause),
                ) as cursor:
                    rows = await cursor.fetchall()
                    for row in rows:
                        if row[0] in registration_ids:
                ...
                # get all team tournament rosters the player is in where registrations
                # have closed and they should be marked as ineligible
                async with db.execute(
                    """
                    SELECT p.registration_id
                    FROM tournament_players p
            ...
            # next we want to automatically add the transferred player to any
            # team tournaments where that team is registered and registrations are open
            async with db.execute(
                """
                SELECT s.registration_id, t.id
                FROM team_squad_registrations s
                ...
                """,
                (True, True, self.roster_id, self.player_id),
            ) as cursor:
                rows = await cursor.fetchall()
                squads: dict[int, int] = {}
                for row in rows:
            ...
            if user_id is not None and leave_team_id and leave_team_id != join_team_id:
                ...
                async with db.execute(
                    ...
                    WHERE m.player_id = ?
                        ...
                        AND m.leave_date IS NULL
                    """,
                    (self.player_id, self.roster_leave_id, leave_team_id),
                ) as cursor:
                    row = await cursor.fetchone()
                    assert row is not None
                    roster_count = row[0]
...
class ToggleTeamMemberBaggerCommand(Command[None]):
    ...
    async def handle(self, db_wrapper: DBWrapper):
        async with db_wrapper.connect() as db:
            async with db.execute(
                """
                ELECT m.id, m.is_bagger_clause, r.game
                FROM team_members m
                JOIN team_rosters r ON m.roster_id = r.id
                WHERE m.roster_id = ?
                    AND m.player_id = ?
                    AND m.leave_date IS ?
                """,
                (self.roster_id, self.player_id, None),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
                ...
                if roster_game != "mkw":
                    raise Problem(
                        "Cannot toggle bagger clause for games other than MKW",
                        status=400,
                    )
            await db.execute(
                "UPDATE team_members SET is_bagger_clause = ? WHERE id = ?",
                (not is_bagger_clause, member_id),
            )
            # if we are registered for a tournament squad for the team we are transferring to as the opposite bagger
            # type, we should flip the player's tournament registration to the new bagger type
            async with db.execute(
                """
                SELECT p.id
                FROM tournament_players p
                ...
                WHERE p.player_id = ?
                    ...
                    AND p.is_bagger_clause = ?
                """,
                (self.player_id, self.roster_id, is_bagger_clause),
            ) as cursor:
                rows = await cursor.fetchall()

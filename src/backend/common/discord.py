...
class DiscordService(ABC):
    ...
    @abstractmethod
    async def refresh_token(self, refresh_token: str) -> DiscordAccessTokenResponse | None:
        pass
...
class DiscordApi(DiscordService):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f'{base_url}/oauth2/token', data=body, headers=headers, auth=auth) as resp:
                if int(resp.status/100) != 2:
                    raise Problem(
                        f"Discord returned an error code while trying to authenticate: {resp.status}")
                resp_bytes = await resp.content.read()
                token_resp = msgspec.json.decode(
                    resp_bytes, type=DiscordAccessTokenResponse)
            user_headers = {
            ...
            async with session.get(f'{base_url}/users/@me', headers=user_headers) as resp:
                if int(resp.status/100) != 2:
                    raise Problem(
                        f"Discord returned an error code while fetching user data: {resp.status}")
                resp_bytes = await resp.content.read()
                discord_user = msgspec.json.decode(
                    resp_bytes, type=DiscordUser)
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f'{base_url}/oauth2/token/revoke', data=data, headers=headers, auth=auth) as resp:
                ...
                if resp.status != 401 and int(resp.status/100) != 2:
                    raise Problem(
                        f"Discord returned an error code: {resp.status}")
    ...
    async def get_avatar(self, discord_id: str, avatar: str) -> bytes:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(avatar_url) as resp:
                if resp.status != 200:
                    raise Problem(
                        f"Failed to fetch Discord avatar: {resp.status}")
